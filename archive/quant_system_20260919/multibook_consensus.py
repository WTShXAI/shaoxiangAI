...
# ============================================================
from pipeline import odds_math
...
def devig_proportional(odds: list[float]) -> np.ndarray:
    """最简去水: 1/o 归一化。(实现收敛到 pipeline/odds_math, 2026-09-18)"""
    return np.array(odds_math.devig_n(odds))


def devig_power(odds: list[float], tol: float = 1e-9) -> np.ndarray:
    """幂法去水: 找 k 使 sum(p_i**k)=1。比比例法更抑 FLB。失败回退比例法。
    (实现收敛到 pipeline/odds_math, 2026-09-18)"""
    return np.array(odds_math.devig_power(odds, tol=tol))
