...
from pipeline.odds_math import devig2 as devig2_ssot
...
def devig(odds_h: float, odds_d: float, odds_a: float):
    """朴素去水: 1/o 归一化 → [p_h, p_d, p_a] (和≈1)。非正赔率返回 None。(委托 odds_math SSoT)"""
    p = devig3(odds_h, odds_d, odds_a)
    return p


def devig2(odds_h: float, odds_a: float):
    """2 项去水(OU 大/小, AH 主/客): 1/o 归一化 → [p_h, p_a]。非正赔率返回 None。(委托 odds_math SSoT)"""
    return devig2_ssot(odds_h, odds_a)
