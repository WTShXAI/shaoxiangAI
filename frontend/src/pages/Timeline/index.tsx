...
export default function TimelinePage() {
  ...
  const fetchTimeline = useCallback(async () => {
    try {
      // Fix-4 08-31: 收敛到统一客户端 (auth/X-API-Key/错误归一), 弃裸 fetch
      const res = await timelineService.getToday()
      const json: TimelineResponse = res.data
  ...
  if (error && !data) {
    return <ApiError message={error} onRetry={fetchTimeline} />
  }
  if (!data) return null
