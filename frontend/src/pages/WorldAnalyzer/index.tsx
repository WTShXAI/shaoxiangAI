...
import { worldAnalyzerService, unwrapData } from '@/services/api'
import type { WorldAnalyzeResult } from '@/types'
...
export default function WorldAnalyzer() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      const r = unwrapData<WorldAnalyzeResult>(res)
      setResult(r)
    } catch (e: unknown) {
      ...
      {result && !result.error && (
        <div className="space-y-3">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <div className={`bg-accent-card border rounded-xl p-4 ${
              edge?.edge_pp != null && edge.edge_pp > 0
                ? 'border-emerald-500/60 shadow-[0_0_14px_rgba(16,185,129,0.15)]'
              ...
              {edge && edge.edge_pp != null ? (
                <div className="space-y-1 text-xs">
                  ...
                  <div className="text-[10px] text-ink-disabled pt-0.5">+EV 判据: 胜率 &gt; 隐含 + 抽水 (IR-17) · <span className="text-amber-400/80">诊断层读数, 非 +EV 判据 (三联赛 OOS 未超越市场基线); 标 +EV 信号框只由天眼 (开盘天眼 +EV 裁判) 判定</span></div>
                </div>
          ...
          {/* 平局风险 (draw_signal 提醒层) */}
          {da && (
            <div className="bg-accent-card border border-surface-border rounded-xl p-4">
              <div className="flex items-center justify-between gap-2 mb-2">
                <div className="text-[13px] font-semibold text-ink-primary">平局风险 (参考, 非拍板)</div>
                <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium flex-shrink-0 ${da.draw_alert ? 'bg-ember-500/15 text-ember-400' : 'bg-surface-dark/60 text-ink-muted'}`}>
                  {da.draw_alert ? '⚠ 触发' : '未触发'} · 阈值 {da.alert_threshold ?? 0.24}
                </span>
              </div>
              <div className="text-xs space-y-1.5">
                <div className="flex items-center gap-2">
                  <span className="text-ink-muted w-16 flex-shrink-0">市场 P(平)</span>
                  <ProbBar p={da.market_p_draw ?? 0} color="bg-frost-400" />
                  <span className="text-ink-primary w-14 text-right flex-shrink-0">{pct(da.market_p_draw)}</span>
                  <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium flex-shrink-0 ${da.level === '偏高' ? 'bg-ember-500/15 text-ember-400' : da.level === '偏低' ? 'bg-frost-400/15 text-frost-400' : 'bg-surface-dark/60 text-ink-muted'}`}>
                    {da.level}
                  </span>
                </div>
                <div className="text-[11px] text-ink-muted">
                  联赛平局率 {pct(da.league_draw_rate)}
                  {da.vs_league_pp != null && (
                    <> · 偏离 <span className={da.vs_league_pp > 0 ? 'text-ember-400' : 'text-frost-400'}>{pp(da.vs_league_pp)}</span></>
                  )}
                  {da.model_p_draw != null && (
                    <> · 模型 P(平) <span className="text-ink-primary">{pct(da.model_p_draw)}</span>
                      {da.model_vs_market_pp != null && <> ({pp(da.model_vs_market_pp)})</>}
                      {da.old_tau_candidate && (
                        <span className="px-1 py-0.5 rounded bg-amber-500/15 text-amber-300 text-[10px] ml-1">模型平局候选池 (τ={da.old_tau ?? 0.25}, 仅诊断)</span>
                      )}
                    </>
                  )}
                </div>
                {da.note && <div className="text-[10px] text-ink-disabled">ⓘ {da.note}</div>}
              </div>
            </div>
          )}
