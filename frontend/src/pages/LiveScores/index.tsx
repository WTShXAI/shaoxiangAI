...
export default function LiveScoresPage() {
  ...
  const [analyze, setAnalyze] = useState<{ home: string; away: string; sportKey?: string; odds?: { h: number; d: number; a: number }; handicap?: { ah_line?: number | string; ah_home?: number; ah_away?: number; ou_line?: number | string; ou_over?: number; ou_under?: number }; liveScore?: { homeGoals: number; awayGoals: number; elapsed?: number }; matchKey?: string; matchState?: number; kickoff?: string; league?: string } | null>(null)
  const onAnalyze = useCallback((h: string, a: string, sportKey?: string, odds?: { h: number; d: number; a: number }, handicap?: { ah_line?: number | string; ah_home?: number; ah_away?: number; ou_line?: number | string; ou_over?: number; ou_under?: number }, liveScore?: { homeGoals: number; awayGoals: number; elapsed?: number }, matchKey?: string, meta?: { matchState?: number; kickoff?: string; league?: string }) => {
    setAnalyze({ home: h, away: a, sportKey, odds, handicap, liveScore, matchKey,
      matchState: meta?.matchState, kickoff: meta?.kickoff, league: meta?.league })
  }, [])
  ...
  return (
    <div className='flex flex-col bg-surface-canvas overflow-hidden' style={{ height: 'calc(100vh - 104px)' }}>
      ...
      {analyze && (
        <MatchAnalysisModal
          home={analyze.home} away={analyze.away} sportKey={analyze.sportKey || ''}
          odds={analyze.odds} handicap={analyze.handicap} liveScore={analyze.liveScore}
          matchKey={analyze.matchKey}
          matchState={analyze.matchState} kickoff={analyze.kickoff} league={analyze.league}
