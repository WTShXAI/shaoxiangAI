...
import { terminalService, rankedService, csTrustCardService, unwrapData } from '@/services/api'
...
export function useMatchAnalysis(props: MatchAnalysisModalProps) {
  const { home, away, sportKey, odds, handicap, initialOdds, initialHandicap, liveScore, focus, matchKey } = props
  ...
  const [trustCard, setTrustCard] = useState<CsTrustCardData | null>(null)
  const [trustCardError, setTrustCardError] = useState<string | null>(null)
  const [trustCardLoading, setTrustCardLoading] = useState(false)
  // 波胆快捷入口 → 默认展开子市场 (含波胆)
  const [showSub, setShowSub] = useState(focus === 'correct_score')
  // 主推视图: 单张分析卡 (ranked + decision 同屏, 不再分 Tab)
  const oipRef = useRef<HTMLDivElement>(null)
  // 波胆聚焦 → 滚动到黄金神瞳 CS 信任卡 (波胆唯一权威模型, 见 index.tsx csRef 包裹)
  const csRef = useRef<HTMLDivElement>(null)

  // 数据就绪 + 聚焦波胆 → 自动滚动到黄金神瞳 CS 信任卡
  useEffect(() => {
    if (card && !loading && focus === 'correct_score' && csRef.current) {
      csRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }
  }, [card, loading, focus])

  // 用 ref 持有最新 props, 避免 setInterval 闭包过期 (弹窗打开期间 liveScore/odds 会持续更新)
  const propsRef = useRef({ home, away, sportKey, odds, handicap, initialOdds, initialHandicap, liveScore, matchKey })
  propsRef.current = { home, away, sportKey, odds, handicap, initialOdds, initialHandicap, liveScore, matchKey }
  ...
  const runAnalysis = useCallback((silent = false) => {
    ...
    // 只要有 1X2 初盘即调用; OU 盘口可选(缺失时后端仅出 1X2 排名, OU 小卡降级为"—")
    // 操盘手CS赔率由后端自动从 GQ.db 回退
    if (odds && odds.h && odds.d && odds.a) {
      ...
      rankedService.predict(home, away, odds, handicap)
        ...
        .finally(() => { if (mountedRef.current) setRankedLoading(false) })
    }
  ...
  // CS 信任卡: 开盘结构概率, 与 live 无关 → 打开时拉一次 (不进 25s 热刷新循环, 避免重复打 WAL 库)
  // 透传 match_key 使后端 _gather_opening_odds 的 OU/AH 初盘回退门控生效 → 拿到完整三盘交叉版信任卡
  useEffect(() => {
    if (!home || !away) return
    setTrustCard(null); setTrustCardError(null); setTrustCardLoading(true)
    csTrustCardService.get({ match_key: matchKey, home, away })
      .then((res) => {
        if (!mountedRef.current) return
        // 响应契约: { ok: boolean, data: CsTrustCardData } (顶层 data 即卡本身)
        const d = unwrapData<CsTrustCardData>(res)
        if (d && (d.found || (d.our_top5 && d.our_top5.length))) setTrustCard(d)
        else setTrustCardError('该场无初盘 CS 数据, 信任卡不显示')
      })
      .catch((e: any) => {
        if (mountedRef.current) setTrustCardError(e?.response?.data?.detail || e?.message || '信任卡获取失败')
      })
      .finally(() => { if (mountedRef.current) setTrustCardLoading(false) })
  }, [home, away, matchKey])

  return { card, cardInitial, loading, error, ranked, rankedError, rankedLoading, trustCard, trustCardError, trustCardLoading, showSub, setShowSub, oipRef, csRef }
}
