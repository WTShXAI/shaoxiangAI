...
export function OperatorCardSection({ card }: { card: TerminalDecisionCard }) {
  ...
  return (
    <div className={`rounded-xl border p-3.5 ${isBet ? 'bg-accent/10 border-accent/20' : 'bg-white/[0.03] border-white/[0.06]'}`}>
      ...
      {oc.evidence && oc.evidence.length > 0 && (
        <ul className="mt-2 space-y-1">
          {oc.evidence.slice(0, 3).map((e, i) => (
            <li key={i} className="text-[11px] text-ink-secondary flex gap-2">
              <span className="font-mono text-accent flex-shrink-0">{i + 1}</span>
              <span>{e}</span>
            </li>
          ))}
        </ul>
    ...
    <div>
      ...
      {card.strategy_signals && card.strategy_signals.length > 0 ? (
        <div className="space-y-2">
          {card.strategy_signals.map((s: StrategySignal, i: number) => (
...
export function SubMarkets({ card, showSub, setShowSub }: {
  ...
  if (!sm || (!sm.ou && !sm.draw)) return null
  return (
    <div>
      <button onClick={() => setShowSub(!showSub)}
        ...
        <span>子市场 / SUB MARKETS (大小球 · 平局共识)</span>
        <span>{showSub ? '▾' : '▸'}</span>
      </button>
      {showSub && (
