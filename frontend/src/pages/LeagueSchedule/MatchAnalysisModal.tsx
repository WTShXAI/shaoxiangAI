...
const dirLabel2 = (d: string | undefined) => {
  if (!d) return '—'
                ...
                <div className="rounded-xl bg-accent/5 border border-accent/10 p-3">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-white/70">{card.multibook_consensus.n_books}庄 · {card.multibook_consensus.n_sharp}sharp</span>
                    </div>
                    {card.multibook_consensus.max_spread_pp > 0 && (
                      <span className="text-[10px] font-mono text-ember-400">离散 {card.multibook_consensus.max_spread_pp}pp</span>
                    )}
                  </div>
              ...
              {card.sub_markets?.correct_score?.rows && card.sub_markets.correct_score.rows.length > 0 && (
                <div>
                  ...
                  {card.sub_markets.correct_score.rows.length > 6 && (
                    <div className="text-[11px] text-white/60 text-center mt-1">+{card.sub_markets.correct_score.rows.length - 6} 项更多 (展开子市场查看)</div>
                  )}
                </div>
              )}
                ...
                <div className={`rounded-xl border p-3 ${card.operator_signals.reversal_risk > 0.5 ? 'bg-ember-500/[0.06] border-ember-500/20' : 'bg-accent/5 border-accent/10'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${card.operator_signals.reversal_risk > 0.5 ? 'bg-ember-500/20 text-ember-300' : 'bg-accent/15 text-accent'}`}>
                        逆转 {Math.round(card.operator_signals.reversal_risk * 100)}%
                      </span>
                    </div>
                    <span className="text-[10px] text-white/60">可靠性 {Math.round(card.operator_signals.operator_reliability * 100)}%</span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
