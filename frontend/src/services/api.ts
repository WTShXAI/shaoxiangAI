...
/** 统一解包后端响应: 兼容 BridgeResponse({ok,data}) 与 ApiResponse({data:{...}})。
 *  收敛调用方散落的 `(res.data as any)?.data || res.data`, 去掉 any 并保留类型安全。 */
export function unwrapData<T = unknown>(res: { data?: unknown }): T {
  const body = res?.data
...
export const liveGoalProbeService = {
   ...
   *  2026-09-01 修复: live 场次(mststi 1-5 / minute>0)放行, 仅未开赛走主流白名单 —
   *  否则周中无主流联赛时列表被滤光(白名单副作用)。 */
      ...
      .then((res) => {
        ...
        if (d && Array.isArray(d.matches)) {
          d.matches = d.matches.filter((m: any) => {
            ...
            const st = Number(m?.mststi ?? 0)
            const isLive = st >= 1 && st < 6
            if (isLive) return true              // 进行中(含中场)一律显示, 冷门也看
            if (Number(m?.minute) > 0) return true  // 兜底: minute>0 视为进行中
            return !isObscureLeague(m?.league, m?.sport_key)  // 未开赛才走白名单
          })
        }
        return res
      }),
...
export const terminalService = {
  ...
  analyze: (
    home: string,
    away: string,
    // Fix-4 08-31: 默认必须传空串 — 后端 sport_key='' 才走普通联赛, 默认 wc
    // (soccer_fifa_world_cup) 会触发 goal_scale 1.35 虚高 λ +12% (已知根因)
    sportKey: string = '',
...
// ============================================
// Timeline 服务 (Fix-4 08-31: 裸 fetch → 统一客户端, 自动带 auth/X-API-Key)
// 后端 /api/timeline/today 直接返回 TimelineResponse 顶层结构 (非 BridgeResponse 包裹)
// ============================================
export const timelineService = {
  getToday: (signal?: AbortSignal) =>
    bridgeApi.get<any>('/api/timeline/today', { signal }),
}
...
// ============================================
// 黄金神瞳 — bridge_service:9000  /api/golden-eye (+ /live)
// 三镜头聚合: 天眼(+EV裁判) / 世界级分析(诊断) / 赛程OU / 终场比分概率分布
// 标 +EV 信号判据: 天眼 edge>0 或 世界级 edge_1x2>=1.0pp (IR-21 不自动下注, IR-30 不标稳赢)
// 后端返回 {success, data:{result|livePayload}} (BridgeResponse 仅取 .data)
// ============================================
export interface GoldenEyeTianYan {
  ok: boolean
  side?: string
  model_prob?: number
...
export interface GoldenEyeWorldEdge {
  side?: string
  win_rate?: number | null
  implied?: number | null
  edge_pp?: number | null
}
export interface GoldenEyeWorld {
  edge_1x2?: GoldenEyeWorldEdge | null
  lean?: string | null
  ou?: {
...
export interface GoldenEyeScoreLine {
  score: string
  prob: number
}
// 两镜分歧 (天眼 +EV裁判 vs 世界级诊断) — 分歧时系统按裁决开关定边(默认世界级), 不甩给人工 (IR-21/IR-30)
export interface GoldenEyeDivergence {
  conflict: boolean
  tianyan_side?: string | null
  world_side?: string | null
  tianyan_edge_pp?: number | null
  world_edge_pp?: number | null
  level?: 'none' | 'soft' | 'hard' | null
  resolution?: 'world_preferred' | 'tianyan_preferred' | null
  resolved_side?: string | null
  note?: string
}
export interface GoldenEyeItem {
  home: string
  away: string
  league?: string | null
  odds?: { h: number; d: number; a: number }
  tianyan: GoldenEyeTianYan
  world: GoldenEyeWorld
  ou: GoldenEyeOU
  score_dist: GoldenEyeScoreLine[]
  score_total?: number
  divergence?: GoldenEyeDivergence | null
  match_key?: string
  price_source?: string
  kickoff?: string | null
}
export interface GoldenEyeLivePayload {
  result: GoldenEyeItem[]
  count: number
  tianyan_covered?: number
  note?: string
  error?: string
}
export interface GoldenEyeAnalyzePayload {
  home: string
  away: string
  sport_key?: string
  odds_h?: number
...
export const goldenEyeService = {
  /** 实时全部非完结比赛三镜头聚合 (标 +EV 信号优先排序) */
  live: () =>
    bridgeApi.get<BridgeResponse<GoldenEyeLivePayload>>('/api/golden-eye/live', { timeout: 60000 }),
  /** 单场三镜头聚合 */
  analyze: (payload: GoldenEyeAnalyzePayload) =>
    bridgeApi.post<BridgeResponse<{ result: GoldenEyeItem; price_source?: string }>>('/api/golden-eye', payload, { timeout: 90000 }),
}

// ── CS 信任卡 (黄金神瞳 build_trust_card) — 分析弹窗 CS 区块接入 (option B) ──
// 结合初盘 1X2+OU+AH+完整CS矩阵 的结构校准分布 + 庄家盘口对照 + 诱导标记 + 信任分。
// 后端 GET /api/cs/trust-card 响应契约: { ok: boolean, data: CsTrustCardData } (顶层 data 即卡本身)。
export const csTrustCardService = {
  get: (params: { match_key?: string; home: string; away: string }) =>
    bridgeApi.get<{ ok: boolean; data: CsTrustCardData }>(
      '/api/cs/trust-card', { params, timeout: 30000 }
    ),
}
