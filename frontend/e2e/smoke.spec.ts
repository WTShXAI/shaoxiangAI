...
test('LiveScores 点击分析打开决策弹窗 (拆分回归)', async ({ page }) => {
  ...
  // 弹窗出现 (全链路决策卡头 + 概率排名主推 Tab; LIVE DECODE 为 8-27 重建前旧文案, 已失效)
  await expect(page.locator('text=全链路决策').first(), '应打开决策弹窗').toBeVisible({ timeout: 25000 })
  await expect(page.locator('text=概率排名主推').first(), '弹窗应有概率排名主推徽标').toBeVisible({ timeout: 25000 })
  // 操盘手结论蒸馏卡 (本次优化新增: 此前已在类型中定义但从未渲染; 后端 terminal/analyze 必返回 operator_card)
  await expect(page.locator('text=操盘手结论').first(), '弹窗应渲染操盘手结论蒸馏卡').toBeVisible({ timeout: 15000 })
