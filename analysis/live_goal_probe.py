...
def probe_core(odds, current_score='0-0', current_minute=0, league=None, con=None, match_key=None, is_halftime=False):
    ...
    for key in odds:
        if key.startswith('OU_1H_') and key.endswith('__over'):
            ...
            if ov is not None and un is not None:
                try:
                    line = _extract_line_from_market(line_key)
                except Exception:
                    line = 0.5
    ...
    for key in odds:
        if key.startswith('OU_1H_') and key.endswith('__over'):
            ...
            if ov and un:
                try:
                    _l = _extract_line_from_market(line_key)
                except Exception:
                    continue
...
def probe_match_with_con(con, match_key, current_score='0-0', current_minute=0, league=None, is_halftime=False):
    ...
    # 2026-08-31 动态发现高线: 硬编码列表只到 4.25, 大比分场(总球已 5+)庄家挂
    # 4.5~8.0 线, 全部候选被"total_now>=line 跳过已破线"清空 → 退回 league_prior
    # (实测 札幌冈萨多 1-4@69' 庄家挂 6.0/6.5/7.5 且实时更新, 模型却无线可用)。
    # 追加该场实有的合法 OU 市场(排除半场/角球/超界线)。
    try:
        _rows = con.execute(
            "SELECT DISTINCT market FROM odds_snapshots "
            "WHERE match_key=? AND market LIKE 'OU_%' "
            "AND market NOT LIKE 'OU_1H%' AND market NOT LIKE 'OU_2H%' AND market != 'OU' "
            "AND captured_at > strftime('%s','now','-2 day')", (match_key,)).fetchall()
        for (_mkt,) in _rows:
            _line = _extract_line_from_market(_mkt)
            if _line is not None and _ok_ou_line_value(_line) and _mkt not in markets:
                markets.append(_mkt)
    except Exception:
        pass
