...
async def predict_image(file: UploadFile = File(...)):
    ...
    async def generate():
        ...
        yield f"data: {json.dumps({'type':'text','content':f'📸 图片已接收 ({file.filename}, {size_mb:.1f}MB)\\n'})}\n\n"
        if size_mb > 10:
        ...
        try:
            ...
            if ocr_used:
                yield f"data: {json.dumps({'type':'text','content':f'✅ 识别完成 ({len(ocr_text)}字符)\\n'})}\n\n"
                try:
                    ...
                    if parse_result.valid_count > 0:
                        ...
                        yield f"data: {json.dumps({'type':'text','content':f'📋 解析: {m.home} vs {m.away}\\n  赔率: {m.odds_h}/{m.odds_d}/{m.odds_a}\\n'})}\n\n"
                        ...
                        yield f"data: {json.dumps({'type':'text','content':'⚠️ 未能从图片中提取到比赛信息\\n\\n建议: 直接用文字描述比赛'})}\n\n"
                except Exception as e:
                    yield f"data: {json.dumps({'type':'text','content':f'⚠️ 解析失败: {e}\\n\\n{ocr_text[:500]}'})}\n\n"
            else:
                yield f"data: {json.dumps({'type':'text','content':f'{ocr_text}\\n\\n💡 建议: 将赔率截图中的文字直接粘贴到输入框分析。'})}\n\n"
        finally:
