...
from pipeline.clean_outcomes import is_clean  # IR-04 SSoT: 假0-0 统一口径
...
def resolve_snapshot(con: sqlite3.Connection, match_key: str, force: bool = False) -> int:
    ...
    干净子集铁律(08-30/IR-04): score_missing=1 的假 0-0 不回填, 避免污染训练集。
    口径统一走 pipeline.clean_outcomes (SSoT, 新库打标+旧库兼容)。
    """
    if not is_clean(con, match_key):
        return 0  # 假 0-0 (score_missing=1) 或查无此场, 不污染训练集
    row = con.execute(
