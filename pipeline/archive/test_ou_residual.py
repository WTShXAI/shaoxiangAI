"""Test OU_residual approach — fundamental S7 math fix"""
...
from rules.d_gate_v52 import get_cover_adjustment, get_similar_odds_warning, COVER_DB
...
def ou_residual(hcp, ou):
    """实际OU - 期望OU. >0.3 = OU异常偏高"""
    ah=abs(hcp)
    expected = 2.25 + 0.5 * ah  # 线性拟合: OU = 2.25 + 0.5*|hcp|
    return ou - expected

def dgate_ou_res(ph,pd,pa,oh,od,oa,hcp,ou,home='',away=''):
    spread=abs(ph-pa);max_imp=max(ph,pa)
    s1=od/math.sqrt(oh*oa);s7=ou/max(abs(hcp),0.25)
    ou_r=ou_residual(hcp,ou)
    cover_mult,_=get_cover_adjustment(home,away) if home else(1.0,'')
    d_boost=None;mode='normal';signals=[]
    ...
    if 0.48<=max_imp<=0.70 and d_boost is None:
        ...
        if ou<=2.5:d*=1.05
        # 🔑 OU_residual penalty: only when OU is genuinely elevated
        if ou_r>=0.3 and s1<1.35:
            d*=0.70
        d*=cover_mult
    ...
    return (v if v!='D'else'D'), mode, d_boost, ou_r

# Run comparison
print('=== OU_residual vs S7绝对值 对比 ===\n')
for label,use_new in [('v5.2.2(S7绝对值)   ',False),('v5.3 (OU_residual) ',True)]:
    ...
    for m in MATCHES:
        ...
        if use_new:
            v,mode,d,ou_r=dgate_ou_res(ph,pd,pa,oh,od,oa,hcp,ou,h,a)
        else:
            from rules.d_gate_v52 import dgate_v52
            v,mode,d,_=dgate_v52(ph,pd,pa,oh,od,oa,hcp,ou,h,a)
            ou_r=ou_residual(hcp,ou)
        ...
        if (act=='D' and v!='D') or (v=='D' and act!='D'):
            changes.append((h,a,act,v,sc,ou_r))
    
    ad=max(1,sum(1 for m in MATCHES if m[6]=='D'))
    df1=2*tp/(tp+fp)*dr/ad/(tp/(tp+fp)+dr/ad) if tp>0 else 0
    print(f'{label}: Acc={c}/38 D-Recall={dr}/13 D-Pred={dr+fp}(FP={fp}) D-F1={df1:.3f}')

# Detailed OU_residual analysis
hdr='比赛                   hcp   ou    S7   OU_r    S1   act  OU异常?'
print(f'\n{hdr}')
for m in MATCHES:
    h,a,oh,od,oa,hcp,ou,act,sc=m
    s1=od/math.sqrt(oh*oa);s7=ou/max(abs(hcp),0.25)
    ou_r=ou_residual(hcp,ou)
    abnormal = '⚠️惩罚' if ou_r>=0.3 and s1<1.35 else ''
    nm=f'{h}vs{a}'
    print(f'{nm:<28} {hcp:>+5.1f} {ou:>4.1f} {s7:>5.1f} {ou_r:>+6.2f} {s1:>5.2f} {act:>4} {abnormal:<8}')

# Test Canada specifically
print(f'\n=== Canada拯救验证 ===')
h,a,oh,od,oa,hcp,ou='加拿大','波黑',1.84,3.45,4.60,-0.5,2.5
r=up.predict(home=h,away=a,odds_h=oh,odds_d=od,odds_a=oa,asian_handicap=hcp,ou_line=ou)
p=r.get('probabilities',{});ph=p.get('H',0);pd=p.get('D',0);pa=p.get('A',0)
v,mode,d,ou_r=dgate_ou_res(ph,pd,pa,oh,od,oa,hcp,ou,h,a)
vmap={'H':'主','D':'平','A':'客'}
chk_icon = 'OK' if v=='D' else 'MISS'
print(f'model_pD={pd:.3f} | OU_residual={ou_r:+.2f} | verdict: {vmap[v]} d={d:.3f} => {chk_icon}')

# Count matches where OU is truly abnormal
abnormal=[(m[0],m[1],ou_residual(m[4],m[5])) for m in MATCHES if ou_residual(m[4],m[5])>=0.3]
print(f'\nOU异常偏高(≥0.3)的场次: {len(abnormal)}')
for h,a,r in abnormal: print(f'  {h}vs{a}: OU_r={r:+.2f}')
