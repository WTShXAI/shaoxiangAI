...
_TEAM_ALIAS = {
    ...
    'canada': '加拿大', 'paraguay': '巴拉圭', 'norway': '挪威',
...
# 用于 "中英同步": 无论调用方传中文还是英文, 都归一到同一英文 canonical key,
# 从而命中同一场比赛行, 避免任一种语言 "查不到".
_ZH2EN = {v: k for k, v in _TEAM_ALIAS.items()}
...
def _canon_team(name: str) -> str:
    """中英同步核心: 任意 中文/英文/别名 输入 -> 英文 canonical 队名。
    英文输入经 _norm_team 转中文键, 再经 _ZH2EN 转回英文 canonical;
    中文输入直接经 _ZH2EN 转英文; 未收录则原样返回(极端兜底)。"""
    if name is None:
        return name
    zh = _norm_team(name)
    return _ZH2EN.get(zh, zh)

DE_THRESHOLD = 0.688  # Youden J 最优阈值 (v3_focal WC重训)
...
def _predict_optimized(match: MatchInput) -> PipelineResult:
    ...
    if features is not None:
        ...
        if _load_de():
            try:
                raw = _DE_PKG['model'].predict_proba([features])[0][1]
    ...
    # 跨届walk-forward证明ML不外推(OOS optimized 0.613<argmax 0.710);
    # 规则层80场回测55%<纯赔率66.2%, ML锁定后唯一可信锚点是市场赔率。
    # 护栏OFF时optimized最终判决取市场argmax, 使生产≥纯赔率(不拖后腿)。
