...
def run_pipeline(model_path: str = 'saved_models/football_ensemble_20260613_150555.joblib',
    ...
    """
    from predictors.selective_predictor import SelectivePredictor
    from predictors.prediction_tracker import PredictionTracker
    ...
    if results:
        ...
        if qualified:
            ...
            for r in qualified:
                ...
                r.predicted_at = datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')
    ...
    report_path = f"output/pipeline_report_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M')}.md"
...
if __name__ == '__main__':
    ...
    if args.audit:
        from predictors.prediction_guard import audit_predictions
        logger.info("🔍 启动独立审计模式...")
        report = audit_predictions()
        guard = __import__('predictors.prediction_guard', fromlist=['PredictionGuard']).PredictionGuard()
