...
组件矩阵 (全部惰性加载, 任一缺失 → None, 零回归):
  ...
  │ 平局提醒层(1.2.0) │ 市场去水 P(D) 主源 + 联赛历史平局率对照 + 分档      │
  └──────────────────┴──────────────────────────────────────────────┘

平局提醒层 (2026-09-01, 用户批准 Task#88 收尾): 实证 direction_model P(D)
平局二元 AUC 0.5776 < 市场去水 0.5825 → 市场赔率即最好平局源, 模型不超越市场;
τ=0.25 候选池 44~68% 入池门槛无效 → 落地为连续值提醒(分档 偏高/中性/偏低),
direction_model P(D) 仅特征齐全时对照, 单场绝不伪造特征硬推(铁律).
...
# ─────────────────────────────────────────────────────────────────
#  平局提醒层 (2026-09-01, 用户批准 Task#88 收尾)
#  复用 pipeline.draw_signal 校准口径 (SSoT, IR-01) —— 7180场回测 + walk-forward
#  39381场优化阈值 DRAW_ALERT=0.24 (精度0.280/召回0.798, 见 draw_signal.py 头注)。
#  09-01 追加实证 (scripts/eval_draw_discrimination_20260901.py, 测试集2025 n=5615):
#    - direction_model P(D) 平局二元 AUC 0.5776 vs 市场去水 0.5825 (Δ-0.005)
#      → 市场赔率即最好平局源, 模型不超越市场 (与 draw_signal"模型lift仅1.08"一致)。
#    - market τ=0.25 池 precision 27.8% ≈ draw_signal 0.24 阈值精度 28.0% → 同源同证。
# 形态: 连续值提醒(market_p_draw + draw_alert布尔 + level分档 + 联赛历史对照),
#      direction_model P(D) 仅特征齐全时对照, 单场绝不伪造特征硬推(铁律)。
# ─────────────────────────────────────────────────────────────────
DRAW_HIGH = 0.30   # 去水平局概率 >= 30% → level 偏高 (辅助描述, 非判定阈值)
DRAW_LOW = 0.20    # 去水平局概率 <= 20% → level 偏低
OLD_TAU = 0.25     # old_tau 平局候选池阈值(原direction_model决策规则, 实证precision≈25%=无判别力, 仅诊断)


def draw_alert(mkt_devig: Optional[List[float]], lg: Optional[dict],
               model_p_draw: Optional[float] = None) -> Optional[dict]:
    """平局提醒层: market_draw_prob 主源 + draw_signal 校准阈值触发 + 联赛对照.
    返回 None = 无 1X2 赔率. 纯参考, 非拍板依据(IR-30)."""
    if not mkt_devig or len(mkt_devig) < 3 or mkt_devig[1] is None:
        return None
    from pipeline.draw_signal import draw_alert_with_booster, DRAW_ALERT
    p_d = mkt_devig[1]
    # 单庄场景(CLI/单源) consensus=None → 用基础阈值 DRAW_ALERT(0.24);
    # 多庄共识 booster 逻辑由调用方(如 bridge draw_signal)自行触发, 本层不重复
    alert = bool(draw_alert_with_booster(p_d, None))
    lg_dr = lg["draw_rate"] if lg else None
    out: Dict = {
        "market_p_draw": round(p_d, 4),
        "draw_alert": alert,
        "alert_threshold": DRAW_ALERT,
        "league_draw_rate": lg_dr,
        "model_p_draw": round(model_p_draw, 4) if model_p_draw is not None else None,
        "vs_league_pp": round((p_d - lg_dr) * 100.0, 2) if lg_dr is not None else None,
        "model_vs_market_pp": round((model_p_draw - p_d) * 100.0, 2)
        if model_p_draw is not None else None,
        "level": ("偏高" if p_d >= DRAW_HIGH else ("偏低" if p_d <= DRAW_LOW else "中性")),
        # old_tau 候选池: 仅当调用方有完整特征传入 model_p_draw 才评估(IR-30 单场不伪造特征),
        # 实证该池 precision≈25%≈无条件平局率 → 只作"模型+市场共振"提示, 不接产
        "old_tau_candidate": bool(model_p_draw is not None and model_p_draw > OLD_TAU),
        "old_tau": OLD_TAU,
        "note": f"阈值复用draw_signal校准(0.24, walk-forward 39381场: 精度0.280/召回0.798); "
                "direction_model P(D) AUC 0.578≈市场0.582 不超越, 提醒仅参考非拍板(IR-30); "
                f"old_tau候选池(P(D)>{OLD_TAU}) precision≈25%无判别力, 仅提示模型+市场共振, 不接产",
    }
    return out


def _direction_p_draw(features: Optional[dict]) -> Optional[float]:
    """direction_model P(D) 推理. 仅当 odds_features 完整特征可用时才推
    (world_analyzer 铁律: 不给模型喂伪造特征, 缺失一律 None)."""
    try:
        if not features or "feat_vec" not in features:
            return None
        import numpy as np
        m = _load_joblib(os.path.join(_ROOT, "models", "direction_model_20260830.joblib"))
        if not m:
            return None
        model = m["model"]
        feats = m["features"]
        vec = np.asarray([features["feat_vec"][f] if f in features["feat_vec"] else 0.0
                          for f in feats], dtype=float).reshape(1, -1)
...
def _cs_qualified() -> Tuple[bool, str]:
    ...
    双闸门 (Fix-3 08-31): ① 模型文件必须存在 — cs_odds_lr.joblib 已于 08-31 清退
    (LR top1 12.4% < 市场 13.1%), 文件缺失直接判未达标, 防止达标即加载失败."""
    if not os.path.exists(_CS_LR_PATH):
        return False, "已下线(模型文件缺失: cs_odds_lr.joblib 已清退), 未参与判定"
    try:
...
def analyze_match(home: str, away: str, league: Optional[str] = None,
                  ...
                  model_p_draw: Optional[float] = None) -> dict:
    t0 = time.time()
    flags: List[str] = []
    ...
    # ── 平局提醒层 (市场锚主源; 模型 P(D) 仅调用方有完整特征时传入, 单场不伪造特征 → 默认 None) ──
    draw = draw_alert(mkt["1x2_devig"], lg, model_p_draw)
...
def print_report(r: dict) -> None:
    ...
    dw = r.get("draw_alert")
    if dw:
        trig = "⚠触发" if dw.get("draw_alert") else "未触发"
        s = (f"\n■ 平局提醒(参考, 非拍板依据): 市场P(平)={dw['market_p_draw']*100:.1f}% "
             f"[{dw['level']} {trig}, 阈值{dw.get('alert_threshold', 0.24)}]"
             + (f" | 联赛历史平局率 {dw['league_draw_rate']*100:.1f}%"
                + (f" (偏离 {dw['vs_league_pp']:+.1f}pp)" if dw["vs_league_pp"] is not None else ""))
             if dw["league_draw_rate"] is not None else "")
        if dw["model_p_draw"] is not None:
            s += f" | 模型P(平)={dw['model_p_draw']*100:.1f}%"
            if dw.get("old_tau_candidate"):
                s += f" ⚠模型平局候选池(P(D)>{dw.get('old_tau', 0.25)}, 仅诊断不接产)"
        print(s)
        if dw.get("draw_alert") or dw["level"] in ("偏高", "偏低"):
            print(f"  ⚠ {dw['note']}")
    if r["honest_flags"]:
        print("\n■ 诚实边界")
...
def backtest(league: str, n: int = 800, since: int = 2015) -> dict:
    ...
    # 平局提醒层对照: 市场去水 P(D) 的平局二元判别力 (direction_model 0.578 < 市场,
    # 回测特征不可可靠重建 → 只评市场, 不推模型)
    p_d_mkt = base_prob[:, 1]
    auc_draw_mkt = float(roc_auc_score((y_np == 1).astype(int), p_d_mkt)) if len(np.unique(y_np)) > 1 else None
    draw_rate = float((y_np == 1).mean())

    # old_tau 候选池诊断锚: 模型 P(D) 回测不可重建, 用市场去水 P(D) 保守代理
    # (direction_model AUC 0.578≈市场 0.582 判别力持平); 量化 τ=0.25 池平局率 vs 无条件平局率
    # → 池平局率≈无条件率即 precision 无增益 = 无判别力 (诊断用, 不接产)
    tau = OLD_TAU
    pool = p_d_mkt > tau
    n_pool = int(pool.sum())
    pool_draw_rate = float((y_np[pool] == 1).mean()) if n_pool else None
    pool_acc = float((np.argmax(base_prob[pool], 1) == y_np[pool]).mean()) if n_pool else None
    ...
    return {
        ...
        "draw_alert_backtest": {
            ...
            "market_auc_draw": round(auc_draw_mkt, 4) if auc_draw_mkt is not None else None,
            "old_tau_pool": {
                "tau": tau, "n": n_pool,
                "pool_draw_rate": round(pool_draw_rate, 4) if pool_draw_rate is not None else None,
                "pool_acc": round(pool_acc, 4) if pool_acc is not None else None,
                "note": "市场P(D)保守代理模型P(D)(AUC持平0.578≈0.582); 池平局率≈无条件率=无判别力, 诊断锚不接产",
            },
            "note": "市场去水P(D)平局二元AUC(方向模型0.578<市场, 故只评市场锚; 参考非拍板)",
        },
        "by_side": by_side,
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    ap.add_argument("--model-p-draw", type=float, default=None,
                    help="direction_model P(平) (可选; 仅调用方有完整特征时传, 单场不伪造特征)")
    ...
    r = analyze_match(args.home, args.away, args.league,
                      ...
                      kickoff=args.kickoff, model_p_draw=args.model_p_draw)
    if args.json:
        print(json.dumps(r, ensure_ascii=False, indent=2))
    else:
        print_report(r)
