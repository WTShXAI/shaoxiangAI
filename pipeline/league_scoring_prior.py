...
_EVENT_MAP = {
    "欧冠": "champions", "冠军联赛": "cn_champions", "champions": "champions",
...
def match_league(live_league: Optional[str]) -> Dict[str, object]:
    ...
    # 2) 欧战聚合匹配
    # ⚠ 子串收窄 (2026-09-02 修复串味 bug):
    #   旧逻辑用 "冠军联赛" in live / "协会" in live 判欧战, 但中冠全称
    #   "中国足协冠军联赛"、中超 "中国足球协会超级联赛" 均含这些子串,
    #   会被误撞 欧冠(聚合)/欧会(聚合), 把中字头业余联赛当成欧洲顶级赛事
    #   (流动性误判 liquid、先验身份错误)。收紧为仅命中真正欧战命名。
    low = live.lower()
    if "欧冠" in live or "欧洲冠军联赛" in live:
        if _agg.get("champions"):
            m, n = _agg["champions"]
            return {"matched_league": "欧冠(聚合)", "mean": m, "n": n, "method": "aggregate"}
    if "欧会" in live or "欧洲协会联赛" in live:
...
# ── 比分分布过度离散 (肥尾) 映射 ──
# 与 LIQUIDITY_WEIGHT 同源: obscure 联赛赔率更噪/球队实力未知/业余队战术纪律差
# → 更易出 0:3 类屠杀, 但独立 Poisson 把极端比分概率压得过低(低估不确定性)。
# 按流动性给比分分布加负二项过度离散(kappa), 让极端比分拿合理概率质量。
# kappa=0 → 纯 Poisson (主流完全不动, 零回归); kappa 越大尾越肥。
# 数值经 scripts/backtest_obscure_overdispersion.py 在 events.db obscure 子集 walkforward 校准:
#   obscure sweep {0.15,0.25,0.35,0.50} → 0.15 最优(logloss 3.1285 vs 原 0.28 的 3.1384,
#   Δ=-0.0099 校准改善, top1 11.5% 不降); mid=0.12 未纳入 sweep, 保持默认不盲改。
OVERDISPERSION_BY_LIQUIDITY = {"liquid": 0.0, "mid": 0.12, "obscure": 0.15}


def overdispersion_for_liquidity(liquidity: Optional[str]) -> float:
    """联赛流动性 → 比分分布过度离散 kappa. 未知/None → 0 (零回归)."""
    if not liquidity:
        return 0.0
    return OVERDISPERSION_BY_LIQUIDITY.get(liquidity, 0.0)
