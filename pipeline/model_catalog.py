...
# ═══════════════════════════════════════════════════════════════
#  RESEARCH_LEGACY —— 研究/对照模型 (2026-08-31 四方向收尾登记)
#  ------------------------------------------------------------
#  这些权重保留在磁盘供诊断/对照, 但**不属于** M1-M7 合法模型集, 不参与生产决策。
#  validate() 不检查本段 —— 仅登记, 防"游离资产"复现。
#  · direction_model_20260830: OOS 5615 场 AUC 0.655≈baseline 无生产价值;
#    重训提权 v1/v2 全败(passed=false); 仅作 world_analyzer P(D) 对照 + old_tau
#    候选池(实证 precision≈25% 无判别力, 仅诊断)。2026-08-31 拍板 LEGACY 保活。
#    (reports/direction_b_oos_eval_20260831.json)
#  · fused_1x2/fused_league: OOS 1244 场验证 1X2 增益 +2.2pp(微小) / league -0.7pp
#    (增益消失), ROI 全负 → 研究/对照定位, 不接管生产。(reports/fused_oos_ext_20260831.md)
#    fused_league 2026-09-01 拍板弃用转归档 (reports/_model_archives/fused_league_20260831_deprecated.joblib);
#    backtest/research 脚本运行时需先从 archives 拷回。
#  · fl_model_ah: 独立 OOS 站稳(AUC 0.834, acc +20.3pp, 1653 场) 已恢复 data/,
#    是 M5 fl_predictor 的 AH 分支工件。(reports/fl_model_ah_oos_20260831.json)
# ═══════════════════════════════════════════════════════════════
RESEARCH_LEGACY: Dict[str, str] = {
    "models/direction_model_20260830.joblib":
        "world_analyzer 诊断对照(惰性加载, 缺文件优雅降级)",
    "models/fused_1x2_20260831.joblib":
        "fused OOS 研究(增益 +2.2pp 微小, ROI 负), 不接管生产",
    "reports/_model_archives/fused_league_20260831_deprecated.joblib":
        "fused OOS 研究(增益 -0.7pp 消失) → 2026-09-01 拍板弃用转归档, 脚本运行需先拷回 models/",
    "data/fl_model_ah.joblib":
