...
def main():
    # Fix-3 08-31 冻结保护: ou_calibrator_20260830 五道关证明参数敏感 (fuse_w=0.30
    # 与验证权重 0.7 不一致, 独立拟合 -2.75% vs 生产 +3.81%) → 已冻结, 仅作纸盘对照。
    # 已冻结模型拒绝覆写, 如需重训须显式删除旧文件后另存新名。
    if os.path.exists(OUT):
        try:
            frozen = joblib.load(OUT).get("frozen", False)
        except Exception:
            frozen = True  # 无法读取视同冻结, 防误覆写
        if frozen:
            print(f"⛔ 拒绝覆写: {OUT} 已冻结 (frozen=True)。"
                  f"该模型已降级为纸盘对照口径, 禁止重训覆写。")
            sys.exit(1)
    if not available():
