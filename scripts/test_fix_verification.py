...
result1 = FullLinkagePipeline.analyze(m1)
...
result2 = FullLinkagePipeline.analyze(m2)
...
result3 = FullLinkagePipeline.analyze(m3)
...
print("=" * 60)
for i, (name, v) in enumerate([('场景1', v1), ('场景2', v2), ('场景3', v3)]):
    print(f"{name}: primary='{v['primary']', secondary='{v['secondary']}'")
    # 检查是否有模糊标签
    if v['primary'] in ['胜', '负', '平']:
        print(f"  ❌ 错误: 存在模糊标签 '{v['primary']}'")
    else:
        print(f"  ✅ 标签清晰")
