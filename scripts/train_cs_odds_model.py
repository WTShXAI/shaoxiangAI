...
输出: reports/cs_odds_models/cs_odds_xgb.joblib / cs_odds_lr.joblib / cs_odds_report.json|txt
      (Fix-3 08-31: 模型已清退 — LR top1 12.4% < 市场 13.1%, 输出改归档目录,
       world_analyzer 不再加载, 脚本仅保留作研究复现工具)

⚠ 下线守卫 (Fix-3 08-31): CS 模型已于 08-31 清退。本脚本落盘产物一律进
reports/cs_odds_models/ 归档目录, 不注册模型白名单、不被任何生产路径加载。
"""
...
# Fix-3 08-31: 模型已清退, 输出进归档目录 (不再污染 models/ 生产区)
_ARCHIVE = os.path.join(ROOT, "reports", "cs_odds_models")
OUT_XGB = os.path.join(_ARCHIVE, "cs_odds_xgb.joblib")
OUT_LR = os.path.join(_ARCHIVE, "cs_odds_lr.joblib")
OUT_JSON = os.path.join(_ARCHIVE, "cs_odds_report.json")
OUT_TXT = os.path.join(_ARCHIVE, "cs_odds_report.txt")
...
def main() -> None:
    t0 = time.time()
    os.makedirs(_ARCHIVE, exist_ok=True)  # Fix-3: 归档目录先行
