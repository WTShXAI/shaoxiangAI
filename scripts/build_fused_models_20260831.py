...
from pipeline.opening_line import build_opening_lines  # Fix-1 08-31: 盘口线 SSoT (IR-01)
...
FIT_FRAC = 0.60  # Fix-1 08-31: 与 verify_poisson_ou_fivegates 对齐 (0.6), 防口径分裂
...
# ----------------------------------------------------------- 收集同步样本
def collect(use_evidence: bool = False):
    # Fix-1 08-31: 盘口线走 build_opening_lines SSoT (IR-01), 弃用自研 opening_ou
    # (旧 opening_ou 用 minute_at 占位垃圾+最低抽水线, 违 IR-01)
    # Fix-2 08-31: use_evidence=False 时改用 score_missing 主口径 (clean_where SSoT,
    #   08-31 GQ 假0-0 治理落地后主口径), 不再要求 score_at 真实快照证据,
    #   样本扩容 (实测 2963 -> 3399 SQL 层)。use_evidence=True 保留旧快照证据口径。
    ...
    for m in matches:
        ...
        if con.execute("SELECT 1 FROM matches WHERE match_key=? AND score_missing=1", (mk,)).fetchone():
            continue
        if use_evidence and not con.execute("SELECT 1 FROM odds_snapshots WHERE match_key=? AND score_at IS NOT NULL "
...
def main():
    ...
    # ===== 1) OU (Fix-1 08-31: 僵尸融合清理) =====
    # fl_model_ou.joblib 已下线(2026-08-31, AUC 0.523 < baseline 0.529 已删),
    # fl_probs()["ou"] 恒 None → 原融合块训练的是退化模型且照 dump fused_ou,
    # 属于"打印 SKIPPED 却仍在产出"的僵尸分支, 整体删除。OU 统一走 poisson(p_over)。
    print("\n=== OU 走纯泊松 (fl_model_ou 已下线, 僵尸融合分支已删 Fix-1) ===")
    ytr = np.array([1 if r["tot"] > r["line"] else 0 for r in tr])
    yte = np.array([1 if r["tot"] > r["line"] else 0 for r in te])
    base_po = np.array([r["poisson_over"] for r in te])
    m_ou = {
        "poisson_AUC": auc(base_po, yte),
        "n_test": len(yte), "base_rate": float(ytr.mean()),
        "note": "纯泊松口径 (Fix-1 08-31: 僵尸 fused_ou 融合已清理, 不再 dump fused_ou_20260831)",
    }
    metrics["models"]["ou_poisson"] = m_ou
...
def build_ou1h():
    ...
    joblib.dump({"ir": ir, "lr": meta, "trained_at": datetime.datetime.now().isoformat(),
                ...
                # Fix-5 08-31: 研究原型(不可部署)不进 models/ 生产区, 归档到 reports/_model_archives
                os.path.join(ROOT, "reports", "_model_archives", "ou1h_model_20260831.joblib"))
    return res
