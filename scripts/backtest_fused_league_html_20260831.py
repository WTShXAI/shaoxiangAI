...
from scripts.build_fused_models_20260831 import (
    collect, fl_probs, league_probs, build_feat, predict_lambdas, p_over, FIT_FRAC
)
...
def fused_ou_pover(fl_ou_pover, poisson_pover):
    return float(poisson_pover)  # 纯泊松口径 (fl_model_ou 已下线, fused_ou 已清退 Fix-5 08-31)
...
def main():
    ...
    # Fix-2 08-31: fl_model_ou 已下线(2026-08-31, AUC 0.523<baseline), fl_probs()["ou"] 恒 None
    # → 基线 AUC 用纯泊松自身(与融合同源), 诚实显示"OU 单口径无融合对比"
    fl_ou_p = np.array([r["poisson_over"] for r in rows])
    ...
    for lg, lst in by_lg.items():
        ...
        f_p = np.array([r["f_ou"] for r in sample]); b_p = np.array([r["poisson_over"] for r in sample])  # Fix-2: fl_ou 恒 None, 基线=泊松
    ...
    out = dict(
        ...
        n=n,
        data_note="时间外OOS(晚40%, FIT_FRAC=0.60), 排除假0-0(IR-04), 三盘口齐全; 仅n>=15联赛进明细表",
...
<div class="sub">生成于 {out['generated_at']} · 时间外 OOS(晚40%，FIT_FRAC=0.60)，融合模型训练于早60%不复用 · 排除假0-0(IR-04) · 三盘口齐全 · 新口径(仅score_missing)</div>
