...
def load_labels():
    ...
    for mk in mks:
        if " vs " not in mk:
            continue
        h, a = mk.split(" vs ", 1)
        # Fix-1 (08-31) IR-04: 假 0-0 场剔除 — matches.score_missing=1 是采集器
        # 未抓到比分的权威标记 (08-30 治理), 其完场比分被回填 0-0, 直入标签会
        # 系统性压低 OU-over (脏组 over 6.68% vs 干净 51.7%)。
        if c.execute("SELECT 1 FROM matches WHERE match_key=? AND score_missing=1",
                     (mk,)).fetchone():
            n_fake += 1
            continue
        r = c.execute(
