...
Get-Content "D:\Architecture\logs\bridge_stdout.log" -Tail 10 -ErrorAction SilentlyContinue
...
function ShowPredict($label, $hteam, $ateam, $oh, $od, $oa, $hcp, $ou) {
  Write-Host ("=== /predict {0} ===" -f $label)
  $body = @{home=$hteam;away=$ateam;odds_h=$oh;odds_d=$od;odds_a=$oa;hcp=$hcp;ou_line=$ou;matchday=3;stage="knockout"} | ConvertTo-Json -Compress
  try {
    $r = Invoke-RestMethod -Uri "http://127.0.0.1:$port/predict" -Method Post -Body $body -ContentType "application/json" -TimeoutSec 40
    $fv = $r.final_verdict
    $v7 = $r.v7_raw
    Write-Host ("  primary: {0}" -f $fv.primary)
