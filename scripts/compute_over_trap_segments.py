# -*- coding: utf-8 -*-
"""Compute exact stats for new OU over-trap segments: line in {2.25,2.5}, over odds in [2.0,2.2).
actual over-hit, implied (dewatered) prob, edge = actual - implied, n."""
import os, sqlite3
...
for line in [2.25, 2.5]:
    ...
    for home, away, sh, sa, op_line in rows:
        ...
        if 2.0 <= ov < 2.2:
            ...
            n += 1; oh += (1 if total > line else 0)
            implied_sum += dewatered(ov, un)
    if n:
        actual = oh/n
        implied = implied_sum/n
        print(f"line {line:g} over[2.0,2.2): n={n} actual_overhit={actual:.4f} implied={implied:.4f} edge={actual-implied:+.4f} calib_pp={100*(actual-implied):+.1f}")
    else:
        print(f"line {line:g}: no samples")
con.close()
