...
P0修复后: trap_low降权 | 已出线→1X2胜平 | 亚盘联动恢复 | 禁止泄露赛果
</p>'''
...
<div class="odds">
  <span>主{m.odds_h:.2f}</span><span class="dr">平{m.odds_d:.2f}</span><span>客{m.odds_a:.2f}</span><span>{_hcp_disp(m)}</span>
</div>
<div class="verdict">
...
<div class="chains">链-1 净胜差{form["goal_diff_advantage"]:+.2f} {form["strength_gap"]} | 链2 D-Gate:{dgate["verdict"]}</div>
        ...
        h += '</div>'
    h += '<div class="footer">🔧 P0修复: trap_low降权 | 已出线→1X2胜平 | 亚盘联动恢复(HCP+OU交叉矩阵) | 不泄露赛果</div>'
    h += '</div></body></html>'
    return h
