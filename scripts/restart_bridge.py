...
if __name__ == "__main__":
    ...
    time.sleep(2)
    # 2026-09-02: 清 bridge_service 字节码缓存 — real worker 跑系统 Python312,
    # 可能沿用编辑前的陈旧 .pyc 导致代码改动(如 CORS)不生效. 重启前强制重编译.
    _pyc = os.path.join(ROOT, "__pycache__", "bridge_service.cpython-312.pyc")
    if os.path.exists(_pyc):
        try:
            os.remove(_pyc); print("cleared stale bridge_service.pyc")
        except Exception as e:
            print("pyc clear skip:", e)
    start_detached()
    if not wait_listen():
        sys.exit(2)
