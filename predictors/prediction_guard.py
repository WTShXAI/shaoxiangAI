...
class PredictionGuard:
    ...
    def guarded_predict(self, sp, days_ahead: int = 7,
        ...
        report_id = hashlib.md5(
            f"guard_{datetime.now(timezone.utc).isoformat()}".encode()
        ).hexdigest()[:12]
    ...
    def _check_matches_available(self, sp, days_ahead) -> Tuple[bool, str, Dict]:
        try:
            ...
            end = (datetime.now(timezone.utc) + pd.Timedelta(days=days_ahead)).strftime('%Y-%m-%d')
    ...
    def run_standalone_audit(self, sp_model_path: str, db_path: str = None) -> GuardReport:
        ...
        report_id = hashlib.md5(f"audit_{datetime.now(timezone.utc).isoformat()}".encode()).hexdigest()[:12]
        self.current_report = GuardReport(
            report_id=report_id,
