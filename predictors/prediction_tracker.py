...
class PredictionTracker:
    ...
    def save_predictions(self, results: List) -> int:
        ...
        for r in results:
            ...
            existing = conn.execute(
                ...
                (int(r.match_id), datetime.now(timezone.utc).strftime('%Y-%m-%d'))
            ).fetchone()
    ...
    def generate_report(self, days_back: int = 30) -> str:
        ...
        lines.append(f"")
        lines.append(f"**报告时间**: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')}")
...
if __name__ == '__main__':
    ...
    if args.action == 'eval':
        ...
        print(report)
        # 同时保存
        report_path = f'output/prediction_report_{datetime.now(timezone.utc).strftime("%Y%m%d_%H%M")}.md'
