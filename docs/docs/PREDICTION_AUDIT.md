...
| 字段 | 类型 | 含义 |
|------|------|------|
| `_model` | string | 使用的模型文件名，固定为 `football_balanced_production.joblib` |
...
3️⃣ 验证模型来源
   → 确认 _model 字段为 "football_balanced_production.joblib"
...
### 🚫 模型缺失检测
- 如果 `football_balanced_production.joblib` 不存在，系统抛出 `ModelNotAvailableError` 并拒绝启动
- 不允许自动回退到其他模型文件
...
1. **看 `_model` 字段**：如果是 `football_balanced_production.joblib`，说明是真实模型输出的
