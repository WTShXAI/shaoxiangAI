...
| ID | 项 | 处置 | 说明 |
|----|----|------|------|
| P1 | **Package B 组装**（赔率 2.32亿行：odds_changes 43.8M + odds_snapshots 122.8M + bak 12.1M/41.8M） | NOW(只读导出) → 交老板外部法律确认(P2-5) → 另售 | 全系统最高价值资产，保底锚 ¥85–110万 的实体支撑。`scripts/p2_build_delivery_package.py` 已引用 odds 表，扩展即可。导出到新 sqlite，**不碰 events.db**。 |
