...
| 文件 | 用途 | 备注 |
|------|------|------|
| `football_v4.1_production.joblib` | **锁定生产模型** (XGBoost + Ridge 集成) | ModelBridge v2.0 强制锁定，不可替换 |
...
```python
LOCKED_MODEL_FILENAME = "football_v4.1_production.joblib"
```

- 默认加载 `saved_models/football_v4.1_production.joblib`
...
- 若 `football_v4.1_production.joblib` 不存在，立即抛出 `ModelNotAvailableError`
...
| 字段 | 类型 | 示例 | 说明 |
|------|------|------|------|
| `_model` | str | `football_v4.1_production.joblib` | 使用的模型文件名 |
...
# 预期输出:
# OK: football_v4.1_production.joblib | 特征数: 90
```
