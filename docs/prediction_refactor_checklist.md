...
### 附: K线窗口 70d→90d 实验 (2026-09-19, 不采纳)

协议同 70d 采纳 (walkforward TimeSeriesSplit(5), n=3881, 多种子确认)。G ensemble LL:
seed42 **1.0207** / seed7 **1.0226** / seed13 **1.0256** (均值 1.0230) vs 70d 基线
1.0264/1.0326/1.0193 (均值 1.0261)。均值 -0.0031 但 seed13 反超 +0.0063, 效应在种子
噪声内 (70d 种子散布 0.0133) → **不采纳, 70d 窗口维持**。证据: reports/candles_window_90d*.json。
复核时机: 下次重训门控触发 (≥100 新定格判定) 时连同更新数据重跑 90d 对照。

**覆盖修复 (2026-09-19, 生产质量瓶颈)**: daily_predictions 首建后永不刷新的泄漏已修 —
monitor/API 每小时 refresh 未开赛行, 临场写入的 K线判定及时应用 (修复前 09-18 K线覆盖
28/155, 另有 42 场判定到位但未应用; 开赛定格语义不变, 已完赛行不回写)。

### 附二: 假0-0污染 — 评估口径修复 (2026-09-19, 重大)

**发现**: 已结算台账平局率 42.9% (0-0 占平局 73%) vs 数据丰富场 29.7% — 比分帧中途断流
的场定格 0-0 后 status 转 finished, 全部结算面被污染 (校准报告/沙盘回放/联赛下钻/漂移基线)。
**判据** (`pipeline/settle.py::credible_1x2`): 全场 0-0 且最后赔率 tick < kickoff+95min →
假 0-0。实证: 0-0 场 78% 终盘<95min (中位 -3min); 1-1 场 97% ≥95min; 通过守卫的 0-0 占全集
~6.9% (与真实足球吻合)。

**修复后真值** (vs 污染口径): K线 LL 1.006→0.960; 产品层 LL 1.092→0.969; 市场大样本
1.043→0.955; **K线 vs 市场 ΔLL -0.018→+0.013 — "K线优于市场"是污染伪信号**, 市场基准实际
略优 (n=329 小样本)。candles 升主赛前锚点的 ≥300 场对照决策必须用守卫后台账。

**A/B 实验 (训练侧, 不采纳)**: 70d 语料 3881 场中 408 场脏标签 (10.5%), 统一干净评估折下
脏标签训练 LL 0.9910 vs 干净训练 0.9914 — Δ 噪声级, 假0-0对训练无害 → 不重训。
...
### 附五: 训练窗口数据边界 (2026-09-21 发现)

odds_changes tick 历史仅自 **2026-08-21** 起 (~31天) — "70d 全语料"实为"31天全语料",
70d/90d 窗口对比实验数据受限空转 (两者取到完全相同的 3949 场)。窗口扩展问题将在
tick 历史自然增长 ~1-2 个月后才有意义。守卫后干净评估基线 (clean-train, 多种子):
seed42 LL 0.9831 / seed7 0.9879 (n=3949, 421 脏标签剔除) — 现任协议的首个干净口径测量。
