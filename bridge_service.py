...
# H7(2026-07-30): CORS 白名单而非 "*" — 防任意站点带凭证跨域请求.
# 默认仅放行同源前端 (从 :9000 加载). 跨机部署设 CORS_ORIGINS 环境变量.
# 2026-09-02: 开发端口 :3000 (npm run dev) 始终兜底追加, 防 dev 跨域被拦死赛程/实时接口.
_CORS_ORIGINS = [o.strip() for o in os.getenv(
    "CORS_ORIGINS", "http://localhost:9000,http://127.0.0.1:9000").split(",") if o.strip()]
for _dev_origin in ("http://localhost:3000", "http://127.0.0.1:3000"):
...
async def _start_background_loops():
    ...
    # 已加载模型 → 毫秒级(稳态仅 ReverseOddsEngine 31ms + fl_predict 11ms)。
    try:
        def _warm_ranked_models():
            try:
                from pipeline.william_inter_model import predict_1x2, predict_total
                predict_1x2(2.0, 3.2, 3.8, 2.0, 3.2, 3.8)
                predict_total(2.0, 3.2, 3.8, 2.0, 3.2, 3.8)
                from pipeline.analysis_center import _analyze_odds
                _analyze_odds(2.0, 3.2, 3.8, 1.93, 1.95, with_neighbors=True)
                from pipeline.reverse_odds_engine import ReverseOddsEngine
                ReverseOddsEngine()  # 触发误定价模型 _MODEL_CACHE 加载
                from pipeline.fl_predictor import predict_from_odds
                predict_from_odds(h=2.0, d=3.2, a=3.8, ou_line=2.5, ou_over=1.93, ou_under=1.95, league="英超", kickoff=None)
                from pipeline.score_model import predict_score
                predict_score("预热", "预热", 2.0, 3.2, 3.8, max_goal=8, goal_scale=1.2)
                logger.info("[飞轮] ranked predictor models warmed (概率排名首请求不再 ~900ms)")
            except Exception as e:
                logger.warning(f"[飞轮] ranked warmup inner failed: {e}")

        _asyncio.create_task(_asyncio.to_thread(_warm_ranked_models))
        logger.info("[飞轮] ranked predictor warmup started (后台)")
    except Exception as e:
        logger.warning(f"[飞轮] ranked predictor warmup failed: {e}")
...
async def health():
    ...
    checks: Dict[str, Any] = {}
    # Fix-2b 08-31: DB 探活含 40M 行 events.db MAX(captured_at) 查询(即使走索引也有毫秒级 IO),
    # async 端点内同步 sqlite 调用会阻塞事件循环 → 整体包 asyncio.to_thread(技能铁律)。
    checks, ok = await asyncio.to_thread(_health_db_probe, checks, ok)
...
def _health_db_probe(checks: Dict[str, Any], ok: bool):
    """同步 DB 探活(在 to_thread 中运行): football_data.db 连通 + events.db 连通/快照新鲜度。
    2026-08-31 Fix-2a 引入的 events.db 探活曾裸同步 SELECT MAX(captured_at) 全索引扫
    40M 行 ~3s, 阻塞事件循环导致 /health 稳定 3-6s → Fix-2b: 包 to_thread + captured_at
    单列索引(建后 MAX 查询 <1ms)。"""
    # DB 连通性 (实际查询, 不只看文件存在)
    db_path = os.path.join(PROJECT_ROOT, "data", "football_data.db")
    if os.path.exists(db_path):
    ...
    # Fix-2 08-31: 生产主库 events.db 探活 + 快照新鲜度。
    # 采集已切 events.db (ws_collector), 旧 health 只看已废弃的 football_data.db —
    # events.db 挂掉仍报 healthy 是监控盲区。连接失败/缺失 = unhealthy;
    # 快照停更时间仅作警示字段 (夜间无比赛属正常, 不置 unhealthy)。
    events_db = os.path.join(PROJECT_ROOT, "data", "events.db")
    if os.path.exists(events_db):
        try:
            ...
            try:
                row = c.execute(
                    "SELECT MAX(captured_at) FROM odds_snapshots WHERE captured_at > 0").fetchone()
                if row and row[0]:
                    age_min = float(max(0.0, (time.time() - row[0]) / 60.0))
                    checks["events_db"] = "connected"
                    checks["events_db_snapshot_age_min"] = round(age_min, 1)
                else:
                    checks["events_db"] = "connected"
                    checks["events_db_snapshot_age_min"] = None
            except Exception as e:
                checks["events_db"] = f"connected, snapshot probe error: {e}"
            c.close()
        except Exception as e:
            checks["events_db"] = f"error: {e}"
            ok = False
    else:
        checks["events_db"] = "missing"
        ok = False
    return checks, ok
...
async def ready():
    ...
    checks = {"engine": False, "db": False, "events_db": False, "odds_db": False}
    ...
    # Fix-2 08-31: 生产主库 events.db 就绪 (采集/分析全链路依赖)
    events_db = os.path.join(PROJECT_ROOT, "data", "events.db")
    if os.path.exists(events_db) and os.access(events_db, os.R_OK):
        checks["events_db"] = True
...
def _live_predict(home, away, oh, od, oa,
                  ...
                  live_anchor: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # 联赛流动性 → 比分分布过度离散 (肥尾校准, 2026-09-02):
    # obscure 联赛赔率更噪/实力未知, Poisson 把 0:3 类屠杀概率压太低; 按流动性加 NB 过度离散,
    # 让极端比分拿合理概率质量。主流(liquid)kappa=0 完全不动 → 零回归。
    # 仅当联赛被识别(有信号)时才注入, 未知联赛保持 0 (零回归, 不臆造)。
    _overdisp = 0.0
    if league:
        try:
            from pipeline.league_scoring_prior import (
                match_league as _ml_fn, classify_liquidity as _cl_fn,
                overdispersion_for_liquidity as _od_fn)
            _overdisp = _od_fn(_cl_fn(league, _ml_fn(league)))
        except Exception:
            _overdisp = 0.0
    ...
    r = predict_score(home_canon, away_canon, oh, od, oa,
                      ...
                      implied_total=_implied_total, overdispersion=_overdisp)
    ...
    if league:
        try:
            ...
            if oip_raw_total > 0:
                ...
                # 重建比分矩阵: 用 score_matrix(kappa=_overdisp) 复用 SSoT 引擎。
                # _overdisp>0 (obscure) 时为负二项肥尾; =0 (主流) 退化为纯 Poisson, 与旧 _poi 行为逐位一致 → 零回归。
                _mg = M.shape[0] - 1
                _M2 = score_matrix(_lh2, _la2, _mg, kappa=_overdisp)
                _M2sum = _M2.sum()
                if _M2sum > 1e-12:
                    M = _M2 / _M2sum
                    r["lh"] = _lh2
    ...
    if home_goals is not None and away_goals is not None and _elapsed is not None and 0 < _elapsed < 90:
        try:
            ...
            _live_anchored = False
            _anchor_note = ""
            # ── 实时滚球盘重新锚定 (2026-09-02 用户洞察) ──
            # 0:1 半场时, 模型只拿赛前 λ 按剩余时间缩放, 没吃实时滚球盘 → 0:3 没被推高。
            # 真实滚球盘(实时 1X2 + 实时 OU + 实时让球)才是当前最真实的"谁还会进"信号:
            #   split 来自实时 1X2(solve_oip, 已含领先/落后+剩余时间), total 来自实时 OU(剩余总球),
            #   二者结合重锚剩余 λ; 实时盘已含剩余时间, 不再乘 T_ratio(避免双重缩放)。
            # 无 live_anchor(非滚球/无快照) → 保持赛前 λ 缩放(零回归)。
            if live_anchor:
                try:
                    _lx = live_anchor.get("x2")
                    _lou = live_anchor.get("ou")
                    if _lx and _lou:
                        _r_live = predict_score(
                            home, away, float(_lx[0]), float(_lx[1]), float(_lx[2]),
                            goal_scale=(WC_OIP_GOAL_SCALE if (is_wc or is_cup) else GENERAL_OIP_GOAL_SCALE),
                            implied_total=None, overdispersion=0.0)
                        _lh_l = float(_r_live.get("lh") or 1.0)
                        _la_l = float(_r_live.get("la") or 1.0)
                        _split = (_la_l / (_lh_l + _la_l)) if (_lh_l + _la_l) > 0 else 0.5
                        _rem_total = max(0.1, float(_lou[0]) - (H + A))  # 实时 OU 全场线 - 已进
                        _rem_h = max(0.01, _rem_total * (1.0 - _split))
                        _rem_a = max(0.01, _rem_total * _split)
                        _live_anchored = True
                        _anchor_note = f" | 实时滚球盘锚定(1X2 split+OU剩余总球={_rem_total:.2f})"
                        T_ratio = 1.0  # 实时盘已含剩余时间, 不再缩放
                except Exception as _lae:
                    logger.warning(f"[inplay] live-anchor 失败(回退赛前λ缩放): {_lae}")
            ...
            if _total > 1e-10:
                ...
                inplay_info = {
                    ...
                    "live_anchored": _live_anchored,
                    "note": (f"In-play 条件裁剪: 当前 {H}-{A}"
                             f" @ {_elapsed}min, 剩余时间比例 {T_ratio:.1%}{_anchor_note}")
                }
...
async def _cs_trust_card_compute(match_key: str, home: str, away: str, actual_score: str,
    ...
    try:
        ...
        # 08-22 v1 + 08-25 形态: 纯开盘三盘拟合(1X2+OU+AH), 不含滚球即时盘模式
        ...
        try:
            ...
            try:
                if cur_score_tuple is not None and _LIVE_GOAL_OK:
                    _pr = await asyncio.to_thread(
                        _live_goal_probe, match_key, current_score=_cs_str,
                        current_minute=(_mn or 0), is_halftime=False)
            ...
            dm = unified_scoreline(h=odds.get("h"), d=odds.get("d"), a=odds.get("a"),
                                   ...
                                   current_score=_cs_str, current_minute=_mn, ou_hint=_ou_hint)
...
async def cs_trust_card_api(match_key: str = "", home: str = "", away: str = "",
    ...
    形态(决定用户信任生死, 见 reports/cs_inducement_analysis.md):
      ...
      - 第三栏历史实证: 真实完赛库每比分线占比(禁硬编码)
    注: 纯开盘三盘拟合形态(2026-08-22 建立 + 08-25 接真实历史频率), 不含滚球即时盘模式。
    本卡为概率估计与盘口对照, 非"AI预测精确比分", 不输出单点答案(trap规避)。
...
async def rollball_analyze_api(match_key: str, score: str = "0-0", minute: int = 0):
    ...
    前端滚球分析页(Rollball)唯一数据源。全部复用现成模型, 零新算法:
      1X2: 开盘(match_outcomes/op_*) vs 即时(最新帧) 去水概率 + 方向直出
      AH:  开盘线 vs 即时线
      OU:  probe full(line/direction/prob/data_source) + ou_anchor 开盘→当前漂移
      CS:  unified_scoreline (SSoT, ou_hint=OU 判定软约束) + 方向直出
      进度: 当前比分/分钟 + 进球轨迹(score_at 去重时间线)
    """
    match_key = _resolve_mk(match_key)
    sh = sa = None
    try:
    ...
    def _worker():
        ...
        try:
            # ── 实时进度 + 进球轨迹 ──
            mrow = gq.execute(
                "SELECT status, minute, score_home, score_away, kickoff, league, home, away "
            ...
            if odds.get("h") and odds.get("d") and odds.get("a"):
                ...
                ph, pd_, pa = _dewater_1x2(odds["h"], odds["d"], odds["a"]) or (None, None, None)
                opening = {"h": odds["h"], "d": odds["d"], "a": odds["a"],
                           "ph": ph, "pd": pd_, "pa": pa,
                           "fav": max(("home", "draw", "away"),
                                      key=lambda k: {"home": ph, "draw": pd_, "away": pa}[k] or 0)}
            out["opening_1x2"] = opening

            # ── 即时盘(滚球帧) 1X2 + OU + AH ──
            live = {}
            try:
                from analysis.live_goal_probe import (
                    _current_inplay_odds, _dewater_1x2 as _dw, _current_inplay_ah_odds)
                cur = _current_inplay_odds(gq, match_key, max(1, minute)) or {}
                if cur.get("x2"):
                    p = _dw(*[float(x) for x in cur["x2"]])
                    if p:
                        live["x2"] = {"odds": list(cur["x2"]), "p": [round(x, 4) for x in p],
                                      "fav": ["home", "draw", "away"][max(range(3), key=lambda i: p[i])]}
                if cur.get("ou"):
                    live["ou"] = {"line": cur["ou"][0], "over": cur["ou"][1], "under": cur["ou"][2]}
                ahc = _current_inplay_ah_odds(gq, match_key, max(1, minute))
                if ahc:
                    live["ah"] = {"line": ahc[0], "home": ahc[1], "away": ahc[2]}
            except Exception as _e:
                logger.warning(f"[rollball] 即时盘: {_e}")
            out["live_odds"] = live

            # ── OU 破蛋判定 + 锚点漂移 ──
            if _LIVE_GOAL_OK:
                try:
                    pr = _live_goal_probe(match_key, current_score=score, current_minute=minute)
                    out["ou"] = {"line": (pr.get("full") or {}).get("line"),
                                 "direction": (pr.get("full") or {}).get("direction"),
                                 "prob": (pr.get("full") or {}).get("prob"),
                                 "signal": (pr.get("full") or {}).get("signal"),
                                 "data_source": (pr.get("full") or {}).get("data_source"),
                                 "half": {"line": (pr.get("half") or {}).get("line"),
                                          "direction": (pr.get("half") or {}).get("direction"),
                                          "prob": (pr.get("half") or {}).get("prob")},
                                 "anchor": ((pr.get("ou_anchor") or {}).get("full")),
                                 "verdict": ((pr.get("ou_anchor") or {}).get("verdict"))}
                except Exception as _e:
            ...
            try:
                ...
                if _tl:
                    try:
                        ...
                        pass
                _cs_str = f"{_cs_sh}-{_cs_sa}" if (_cs_sh is not None and _cs_sa is not None) else ""
                dm = unified_scoreline(h=odds.get("h"), d=odds.get("d"), a=odds.get("a"),
                                       ou_line=odds.get("ou_line"), ou_over=odds.get("ou_over"),
                                       ou_under=odds.get("ou_under"),
                                       ah_line=odds.get("ah_line"), ah_home=odds.get("ah_home"),
    ...
    try:
        ...
        logger.error(f"[rollball/analyze] 失败: {e}")
...
# 2026-09-02 修正: 原 AH/OU 与 opening 缓存"按 match_key 永久缓存", 导致采集器赛前回填
# 写入新快照后, 运行中的 bridge 仍返回首查时缓存的 None → 未开赛比赛永久缺 AH/OU。
# 改为 TTL 缓存(与 _gq_covered_leagues 同款 300s), 让回填后的新盘口能被读到。
_GQ_AHOU_TTL = 300

def _gq_opening_odds(match_keys):
    """初盘赔率 = 该 match 的第一条 1X2 采集 (captured_at 最小). 按 match_key TTL 缓存(初盘基本不变, 但需可刷新以防回填滞后)."""
    import time as _gt
    _now = _gt.time()
    missing = [k for k in match_keys
               if (k not in _GQ_OPENING_CACHE) or (_now - _GQ_OPENING_CACHE[k][1] > _GQ_AHOU_TTL)]
    if missing:
        try:
            import sqlite3, os
            db = os.path.join(PROJECT_ROOT, "data", "events.db")
            ...
            for mk in missing:
                ...
                _GQ_OPENING_CACHE[mk] = (
                    ({"h": v.get("home"), "d": v.get("draw"), "a": v.get("away")}, _now)
                    if v else (None, _now)
                )
            c.close()
        except Exception:
            for mk in missing:
                _GQ_OPENING_CACHE[mk] = (None, _now)
    return {k: _GQ_OPENING_CACHE[k][0] for k in match_keys
            if isinstance(_GQ_OPENING_CACHE.get(k, (None, 0))[0], dict)}
...
def _gq_best_ah_ou(match_keys):
    ...
    返回 {mk: {ah_line, ah_home, ah_away, ah_op_home, ah_op_away,
             ou_line, ou_over, ou_under, ou_op_over, ou_op_under}}
    按 match_key TTL 缓存(初盘/线不变, 但需可刷新以防赛前回填滞后导致永久缺 AH/OU).
    """
    import time as _gt
    _now = _gt.time()
    missing = [k for k in match_keys
               if (k not in _GQ_AHOU_CACHE) or (_now - _GQ_AHOU_CACHE[k][1] > _GQ_AHOU_TTL)]
    if missing:
        try:
            import sqlite3, os
            db = os.path.join(PROJECT_ROOT, "data", "events.db")
            ...
            for mk in missing:
                ...
                _GQ_AHOU_CACHE[mk] = (entry if entry else None, _now)
        except Exception:
            for mk in missing:
                _GQ_AHOU_CACHE[mk] = (None, _now)
    return {k: _GQ_AHOU_CACHE.get(k, (None, 0))[0]
            for k in match_keys if _GQ_AHOU_CACHE.get(k, (None, 0))[0]}
...
async def terminal_matches_api():
    """[DEPRECATED Fix-5 08-31] 当天可决策比赛列表 — 曾从 live_odds_raw 筛选多庄赔率比赛。
    雷速采集已删 (2026-08-31), football_data.db.live_odds_raw 已无新数据; 前端已无引用
    (赛事列表统一走 /api/terminal/analyze + /api/all-fixtures)。保留端点仅防外部脚本 404,
    返回空列表 + deprecated 标记。"""
    import sqlite3
    try:
        db_path = os.path.join(PROJECT_ROOT, "data", "football_data.db")
        conn = sqlite3.connect(db_path)
        conn.row_factory = sqlite3.Row
        # 表已删/空 → 直接返回 deprecated 空列表 (防每请求 500)
        has_table = conn.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='live_odds_raw'").fetchone()
        if not has_table:
            conn.close()
            return {"ok": True, "matches": [], "mode": "deprecated", "reason": "live_odds_raw 已随雷速下线"}
...
@app.get("/api/world-analyze")
async def world_analyze_api(home: str = "皇家马德里", away: str = "巴塞罗那",
                            league: str = "", h: float = None, d: float = None, a: float = None,
                            ou_line: float = None, ou_over: float = None, ou_under: float = None,
                            ah_line: float = None, ah_home: float = None, ah_away: float = None,
                            op_h: float = None, op_d: float = None, op_a: float = None,
                            kickoff: str = "",
                            model_p_draw: float = None):
    """世界级足球分析器 v1.2 — 市场锚+模型矩阵+一致性+Edge三件套+漂移+联赛背景+平局提醒+诚实边界.
    只读分析, GET 语义; 前端 WorldAnalyzer 页调用. 缺赔率时自动尝试从 events.db 按对阵回填 1X2.
    model_p_draw: 可选 direction_model P(平), 仅调用方有完整特征时传(单场不伪造特征铁律, 缺失=跳过old_tau候选池)."""
    try:
        import sys as _sys
        ...
        # 缺 1X2 赔率 → 尝试 events.db 回填 (obscure 联赛主数据源)
        if not (h and d and a):
            gq = _gq_lookup_1x2(home, away)
            if gq:
                h = gq["h"]; d = gq["d"]; a = gq["a"]
                if not league:
                    league = gq.get("league") or league

        # CS 波胆赔率 (赛前 26 选, 达标闸门模型用; 查不到则 cs_odds=None → 模块标注跳过)
        cs_odds = _gq_lookup_cs(home, away)

        r = analyze_match(home, away, league or None,
                          h=h, d=d, a=a,
                          ou_line=ou_line, ou_over=ou_over, ou_under=ou_under,
                          ah_line=ah_line, ah_home=ah_home, ah_away=ah_away,
                          op_h=op_h, op_d=op_d, op_a=op_a,
                          kickoff=kickoff or None,
                          cs_odds=cs_odds,
                          model_p_draw=model_p_draw)
        return _wrap_data(r)
    except Exception as e:
        logger.error(f"[world-analyze] 分析失败: {e}")
        return _wrap_data({"error": f"分析失败: {e}", "match": {"home": home, "away": away}})


def _gq_lookup_cs(home: str, away: str):
    """events.db 按 home/away 查 CS 波胆赛前最新赔率 (26 选: 0:0..4:4+其他, IR-02 英文冒号).

    返回 {scoreline: odds} 或 None。仅赛前快照 (score_at 空) + captured_at <= kickoff (CS 铁律仅赛前采集)。
    与 scripts/build_cs_dataset.latest_prematch 同语义: 按 captured_at DESC 每 selection 取最新。
    """
    try:
        import os
        import sqlite3
        ...
        if not os.path.exists(db):
            return None
        c = sqlite3.connect(db)
        mk = c.execute("SELECT match_key, kickoff FROM matches WHERE home=? AND away=?",
                       (home, away)).fetchone()
        if not mk:  # 主客颠倒也试一次
            mk = c.execute("SELECT match_key, kickoff FROM matches WHERE home=? AND away=?",
                           (away, home)).fetchone()
        ...
        if kickoff:
            ...
            for fmt, tz in [("%Y-%m-%d %H:%M", TZ8), ("%Y-%m-%dT%H:%M:%SZ", timezone.utc),
                ...
                try:
                    ko_ts = datetime.strptime(kickoff.strip(), fmt).replace(tzinfo=tz).timestamp()
                    break
                except Exception:
                    continue
        cap_lim = ko_ts if ko_ts is not None else 9e18
        rows = c.execute(
            "SELECT selection, odds FROM odds_snapshots WHERE match_key=? "
            "AND market LIKE 'CS%' AND (score_at IS NULL OR score_at='') "
            "AND captured_at <= ? ORDER BY captured_at DESC", (match_key, cap_lim)).fetchall()
        c.close()
        latest = {}
        for sel, odds in rows:
            latest.setdefault(sel, odds)
        needed = [f"{h}:{a}" for h in range(5) for a in range(5)] + ["其他"]
        if all(s in latest for s in needed):
            try:
                out = {s: float(latest[s]) for s in needed}
                if all(0 < o <= 1000 for o in out.values()):
                    return out
            except (TypeError, ValueError):
                return None
        return None
...
def _gq_lookup_1x2(home: str, away: str):
    """events.db 按 home/away 查 1X2 盘口 (obscure 联赛的回退数据源)。

    返回 {"h":float,"d":float,"a":float,"league":str} 或 None。

    稳健性 (2026-09-02 修复): 原实现取单条最新快照 (ORDER BY captured_at DESC LIMIT 1/selection),
    会吃到 obscure 联赛实时盘的脏波动 —— 实测同分钟出现 home 8.9/38.0、draw 1.03/1.44 等
    矛盾值, 直接喂出 "平局 67.8%" 类伪造分析。现改为:
      ① 取最近窗口 (LAST 60 条) 各 selection 的**中位数**, 抑制单条脏快照;
      ② 抽水 (overround) sanity 校验, 真实盘应在 [0.01, 0.30]; 超出说明快照集矛盾/脏 → 返回 None
         (诚实 PASS, 不伪造分析, IR-30)。
    仅当三方盘口齐全、为正、且抽水合理时才返回。
    """
    try:
        ...
        rows = c.execute(
            "SELECT selection, odds FROM odds_snapshots WHERE match_key=? AND market='1X2' "
            "ORDER BY captured_at DESC LIMIT 60", (match_key,)).fetchall()
        c.close()
        ...
        for sel, odds in rows:
            try:
                ...
                pass
        if not (series.get("home") and series.get("draw") and series.get("away")):
            return None
        h = statistics.median(series["home"])
        d = statistics.median(series["draw"])
        a = statistics.median(series["away"])
        # 抽水 sanity: 真实盘抽水应在 [0.01, 0.30]; 超出说明快照集矛盾/脏
        overround = (1.0 / h + 1.0 / d + 1.0 / a) - 1.0
        if not (0.01 <= overround <= 0.30):
            return None
        # 矛盾集 sanity: obscure 联赛实时盘多源冲突时, 会出现"一方赔率近 1.0(隐含~99%)
        # 但另两方隐含仍不低"的自相矛盾快照 (实测 draw 1.01 + away 7.1)。单方隐含 >0.90
        # 却次高方隐含仍 >0.10 → 不可能存在的盘, 直接拒 (诚实 PASS, 不伪造方向)。
        _imp = sorted([1.0 / h, 1.0 / d, 1.0 / a], reverse=True)
        if _imp[0] > 0.90 and _imp[1] > 0.10:
            return None
...
async def terminal_analyze_api(req: TerminalAnalyzeRequest):
    ...
    try:
        ...
        # ── 实时滚球盘锚定 (2026-09-02 用户洞察): in-play 时从 odds_snapshots 取真实滚球盘,
        #    重锚剩余 λ (split 来自实时 1X2, total 来自实时 OU), 让分布反映市场真实倾向(如 0:1→0:3) ──
        _live_anchor = None
        if (req.home_goals is not None and req.away_goals is not None
                and req.elapsed is not None and 0 < int(req.elapsed) < 90):
            try:
                from analysis.live_goal_probe import _current_inplay_odds, _current_inplay_ah_odds
                import sqlite3 as _sq
                _mk = f"{req.home} vs {req.away}"
                _con = _sq.connect(os.path.join(PROJECT_ROOT, "data", "events.db"))
                _con.row_factory = _sq.Row
                _cur = _current_inplay_odds(_con, _mk, int(req.elapsed))
                _ah = _current_inplay_ah_odds(_con, _mk, int(req.elapsed))
                _con.close()
                if _cur and _cur.get("x2") and _cur.get("ou"):
                    _live_anchor = {"x2": _cur["x2"], "ou": _cur["ou"], "ah": _ah}
            except Exception as _lae:
                logger.warning(f"[terminal/analyze] live-anchor 取数失败(回退赛前λ缩放): {_lae}")
...
@app.post("/api/open-eye/recommend")
async def open_eye_recommend_api(req: TerminalAnalyzeRequest):
    """开盘天眼 +EV 裁判 (2026-08-31 部署, IR-20/IR-21/IR-30)。

    消费 pipeline.open_eye_predictor.recommend: 模型P - 开盘去水隐含P 最大方为建议边;
    覆盖门(两队均入 team_canonical)不达标 或 edge<=0 -> 返回 ok=False(天眼 PASS, 诚实边界)。
    仅建议, 不自动下注(IR-21); 不标稳赢(IR-30)。

    2026-09-01 修复(输入价语义): 天眼特征 odds_extra 的语义是**开盘价**(仅开盘派生, 无前视),
      而前端此前传最新快照价 / 后端回退 _gq_lookup_1x2 也是最新价 -> 训练/推理分布不一致 + 前视泄漏。
      现在: ① 按 home/away 查 match_key -> _gq_opening_odds(MIN captured_at=初盘) 优先;
            ② 查不到初盘且前端显式传了开盘价(opening_*) -> 用 provided;
            ③ 两者皆无 -> PASS, 绝不用滚球价冒充开盘价(IR-30 诚实边界)。
    """
    try:
        from pipeline.open_eye_predictor import recommend as oe_recommend

        oh, od, oa, price_source = None, None, None, None

        # ① 优先初盘: events.db 按 home/away 找 match_key -> MIN(captured_at) 1X2
        try:
            import sqlite3 as _sq
            _db = os.path.join(PROJECT_ROOT, "data", "events.db")
            if os.path.exists(_db):
                _c = _sq.connect(_db)
                _mk = _c.execute(
                    "SELECT match_key FROM matches WHERE home=? AND away=? LIMIT 1",
                    (req.home, req.away)).fetchone()
                if not _mk:
                    _mk = _c.execute(
                        "SELECT match_key FROM matches WHERE home=? AND away=? LIMIT 1",
                        (req.away, req.home)).fetchone()
                _c.close()
                if _mk:
                    _op = _gq_opening_odds([_mk[0]]).get(_mk[0])
                    if _op and _op.get("h") and _op.get("d") and _op.get("a"):
                        oh, od, oa = float(_op["h"]), float(_op["d"]), float(_op["a"])
                        price_source = "opening_gq"
        except Exception:
            pass

        # ② 前端显式传开盘价 (调用方语义约定: odds_h/d/a 传 opening_*, 见 useSchedule.ts)
        if oh is None and (req.odds_h and req.odds_d and req.odds_a
                           and req.odds_h > 0 and req.odds_d > 0 and req.odds_a > 0):
            oh, od, oa = float(req.odds_h), float(req.odds_d), float(req.odds_a)
            price_source = "provided"

        # ③ 无开盘价 -> PASS (绝不喂滚球价)
        if oh is None:
            return _wrap_data({
                "result": {"ok": False, "reason": "无开盘盘口(初盘缺失), 天眼 PASS (IR-30 诚实边界; 滚球价不冒充开盘价)"},
                "price_source": "none"})

        league_name = (LEAGUE_CATALOG.get(req.sport_key, {}) or {}).get('name', req.sport_key) or None
        rec = oe_recommend(req.home, req.away, oh, od, oa, None, league_name)
        rec["price_source"] = price_source
        return _wrap_data({"result": rec, "price_source": price_source})
    except Exception as e:
        return _wrap_data({
            "result": {"ok": False, "reason": "天眼推理异常: %s" % str(e)},
            "price_source": "error"})


@app.post("/api/best-combo/analyze")
async def best_combo_api(req: TerminalAnalyzeRequest):
    """4 盘口诚实综合分析与候选信号 (2026-08-31, IR-30)。

    复用 TerminalAnalyzeRequest 赔率字段; 解析盘口线后调 analysis.best_combo.analyze_best_combo。
    输出 胜平负/大小球/让球/波胆 四条件综合分析, 所有 edge 标注来自收盘现实价压测证据;
    候选信号未部署、未真实下注(IR-20/IR-21/IR-30)。
    """
    try:
        import sqlite3
        ...
        # 赔率回退(与 terminal/analyze 同源): 前端未传 → GQ 单源
        if not (oh and od and oa and oh > 0 and od > 0 and oa > 0):
            gq = _gq_lookup_1x2(req.home, req.away)
            if gq:
                oh, od, oa = gq["h"], gq["d"], gq["a"]
                ...
                return _wrap_data({"error": f"未找到盘口赔率: {req.home} vs {req.away}", "result": None})
        ...
        ou_line = _parse_handicap_line(req.ou_line)
        ah_line = _parse_handicap_line(req.ah_line)
        result = analyze_best_combo(
            home=req.home, away=req.away, oh=oh, od=od, oa=oa, league=league_name,
            line=ou_line, ov=req.ou_over, un=req.ou_under,
            ah_line=ah_line, ah_h=req.ah_home, ah_a=req.ah_away, ko=None,
        )
        return _wrap_data({"result": result})
    except Exception as e:
        return _wrap_data({"error": str(e), "result": None})


# ═══ 黄金神瞳 (Golden Eye) — 三镜头聚合 + 实时绿框 + 比分概率分布 ═══
def _golden_score_dist(oh, od, oa, top=6):
    """1X2 去水 → DC 反演 λ → 泊松比分概率分布 (诚实: 分布非准确预测)."""
    try:
        from pipeline.open_eye_predictor import _pois, _dc_inv
        ...
        for i in range(6):
            for j in range(6):
                grid[(i, j)] = _pois(lh, i) * _pois(la, j)
        total = sum(grid.values()) or 1.0
        ranked = sorted(grid.items(), key=lambda x: -x[1])[:top]
        return [{"score": f"{i}:{j}", "prob": round(p / total, 4)} for (i, j), p in ranked], round(total, 4)
    except Exception:
        return [], None



def _golden_eye_one(home, away, league, oh, od, oa):
    """单场三镜头聚合 (诊断, 非下注). 返回 {tianyan, world, ou, score_dist, score_total, divergence}."""
    out = {"home": home, "away": away, "league": league,
           "odds": {"h": oh, "d": od, "a": oa}, "tianyan": None, "world": None,
           "ou": None, "score_dist": [], "score_total": None}
    # 天眼 (开盘独立模型, 验证+EV) — 指定 +EV 裁判
    ty_side = ty_edge = None
    try:
        from pipeline.open_eye_predictor import recommend as oe_recommend, _covered
        if _covered(home, away):
            rec = oe_recommend(home, away, oh, od, oa, None, league)
            out["tianyan"] = rec
            if rec.get("ok"):
                ty_side = rec.get("side")
                ty_edge = rec.get("edge_pp", 0)
        else:
            out["tianyan"] = {"ok": False, "reason": "覆盖不足(至少一队无可靠独立实力历史), 天眼 PASS (IR-30)"}
    except Exception as e:
        out["tianyan"] = {"ok": False, "reason": f"天眼异常: {e}"}
    # 世界级分析 (诊断 edge 三件套) — 诊断层, 非拍板机
    we_side = we_edge = None
    try:
        from pipeline.world_analyzer import analyze_match
        wa = analyze_match(home, away, league, h=oh, d=od, a=oa)
        e = wa.get("edge_1x2")
        out["world"] = {"edge_1x2": e, "lean": wa.get("consensus", {}).get("lean"),
                       "ou": wa.get("ou"), "version": wa.get("version")}
        if e and e.get("edge_pp") is not None:
            we_side = e.get("side")
            we_edge = e["edge_pp"]
    except Exception as e:
        out["world"] = {"error": str(e)}

    # ── 两镜分歧检测 (诊断, 非下注; 诚实边界 IR-21/IR-30) ──
    # 天眼=独立实力模型判+EV; 世界级=诊断矩阵判+EV. 二者 side 不一致=方向打架.
    # HARD: 两镜各自断言+EV 且方向相反 → 系统按世界级口径裁决, 仅展示分歧诊断(不标绿框/不代下注).
    # SOFT: 仅一镜+EV 但方向相反 → 以+EV裁判(天眼)为准, 展示分歧提醒.
    divergence = {
        "conflict": False,
        "tianyan_side": ty_side,
        "world_side": we_side,
        "tianyan_edge_pp": round(ty_edge, 2) if isinstance(ty_edge, (int, float)) else None,
        "world_edge_pp": round(we_edge, 2) if isinstance(we_edge, (int, float)) else None,
        "level": "none",
        "note": "",
    }
    if ty_side and we_side and ty_side != we_side:
        divergence["conflict"] = True
        if (ty_edge or 0) > 0 and (we_edge or 0) >= 1.0:
            divergence["level"] = "hard"
            divergence["note"] = (
                f"两镜方向相反且各自+EV: 天眼→{ty_side}(+{ty_edge:.1f}pp) / 世界级→{we_side}(+{we_edge:.1f}pp)"
                f"; 系统按世界级口径裁决 (IR-21 不自动下注, 仅展示分歧诊断)"
            )
        else:
            divergence["level"] = "soft"
            divergence["note"] = (
                f"两镜方向不一致: 天眼→{ty_side}({(ty_edge or 0):+.1f}pp) / 世界级→{we_side}({(we_edge or 0):+.1f}pp)"
                f", 以+EV裁判(天眼)为准, 注意分歧"
            )
    out["divergence"] = divergence
    # 比分概率分布 (诚实: 非准确预测)
    sd, st = _golden_score_dist(oh, od, oa)
    out["score_dist"] = sd
    out["score_total"] = st
    return out


@app.post("/api/golden-eye")
async def golden_eye_api(req: TerminalAnalyzeRequest):
    """黄金神瞳 · 单场三镜头聚合 (天眼+EV / 世界级分析诊断 / 赛程OU / 比分分布)."""
    try:
        oh, od, oa, price_source = None, None, None, None
        try:
            import sqlite3 as _sq
            ...
            if os.path.exists(_db):
                _c = _sq.connect(_db)
                _mk = _c.execute("SELECT match_key FROM matches WHERE home=? AND away=? LIMIT 1",
                                (req.home, req.away)).fetchone()
                if not _mk:
                    _mk = _c.execute("SELECT match_key FROM matches WHERE home=? AND away=? LIMIT 1",
                                    (req.away, req.home)).fetchone()
        ...
        if oh is None and (req.odds_h and req.odds_d and req.odds_a
            ...
            price_source = "provided"
        if oh is None:
            return _wrap_data({"result": {"reason": "无开盘盘口, 黄金神瞳 PASS (IR-30)"},
                               "price_source": "none"})
        league_name = (LEAGUE_CATALOG.get(req.sport_key, {}) or {}).get('name', req.sport_key) or None
        res = _golden_eye_one(req.home, req.away, league_name, oh, od, oa)
        res["price_source"] = price_source
        return _wrap_data({"result": res, "price_source": price_source})
    except Exception as e:
        return _wrap_data({"result": {"reason": f"黄金神瞳异常: {e}"}, "price_source": "error"})


@app.get("/api/golden-eye/live")
async def golden_eye_live_api():
    """黄金神瞳 · 实时全部非完结比赛三镜头聚合 (供导航页列表)."""
    try:
        import sqlite3 as _sq
        _db = os.path.join(PROJECT_ROOT, "data", "events.db")
        if not os.path.exists(_db):
            return _wrap_data({"result": [], "note": "events.db 不存在"})
        _c = _sq.connect(_db); _c.row_factory = _sq.Row
        mks = [r[0] for r in _c.execute(
            "SELECT DISTINCT m.match_key FROM matches m JOIN odds_snapshots s "
            "ON s.match_key=m.match_key WHERE m.status NOT IN ('finished') AND s.market='1X2'").fetchall()]
        _c.close()
        opens = _gq_opening_odds(mks)
        ahou = _gq_best_ah_ou(mks)
        results = []
        for mk in mks:
            op = opens.get(mk)
            if not (op and op.get("h") and op.get("d") and op.get("a")):
                continue
            parts = [x.strip() for x in mk.split(" vs ")]
            h, a = (parts + [None, None])[:2]
            if not (h and a):
                continue
            league = None
            kickoff = None
            try:
                _cc = _sq.connect(_db); _cc.row_factory = _sq.Row
                lr = _cc.execute("SELECT league, kickoff FROM matches WHERE match_key=? LIMIT 1", (mk,)).fetchone()
                if lr:
                    league = lr["league"]
                    kickoff = lr["kickoff"] if lr["kickoff"] else None
                _cc.close()
            except Exception:
                pass
            one = _golden_eye_one(h, a, league, float(op["h"]), float(op["d"]), float(op["a"]))
            one["match_key"] = mk
            one["kickoff"] = kickoff
            one["ou"] = ahou.get(mk)
            results.append(one)
        results.sort(key=lambda r: -((r.get("tianyan") or {}).get("edge_pp") or 0))
        return _wrap_data({"result": results, "count": len(results),
                           "tianyan_covered": sum(1 for r in results if (r.get("tianyan") or {}).get("ok"))})
    except Exception as e:
        return _wrap_data({"result": [], "error": str(e)})
