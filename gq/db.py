...
def upsert_match(match_key: str, home: str, away: str, league: str = "",
    ...
    with conn() as c:
        ...
        if _row is None:
            ...
            # ── 防 clobber 扩展 (2026-08-31): league 只在新值非空时更新 ──
            # 实证: 8-29~31 token 失效窗口, 列表接口降级返回 tn='' → 无条件 league=? 覆盖
            # 会把已入库的联赛名清空(波鸿/拜仁等主流联赛全空)。league 是低变值, 空值覆盖
            # 属数据丢失, 宁保留旧值也不清空 (与 HT 防 clobber 同哲学)。
            _lg_row = cur.execute(
                "SELECT league FROM matches WHERE match_key=?", (match_key,)).fetchone()
            _keep_lg = league if (league or "").strip() else (_lg_row[0] if _lg_row else "")
            c.execute("""UPDATE matches SET
                league=?, kickoff=?, status=?, score_home=?, score_away=?,
                ht_score_home=?, ht_score_away=?, minute=?, last_seen=?
                WHERE match_key=?""",
                (_keep_lg, kickoff, status, score_home, score_away,
