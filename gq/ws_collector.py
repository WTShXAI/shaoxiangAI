...
# 临场早盘补采 (2026-08-28 起): WS 不推未开赛场盘口, 用详情 HTTP 接口补采。
# 2026-09-02 修正: 原 PREMATCH_WINDOW_SEC=7200(仅开赛前2h)过窄 —— 实证 leyu 对远未开赛
# 比赛(如克甲U19, kickoff 还有 ~15h)现在就返回完整开盘赔率(code=0000000), 但旧窗口把它们
# 永远挡在补采队列外 → events.db 0 快照 → /api/all-fixtures 无 odds → 前端"未开赛无赔率"。
# 改为两档: 临场(≤2h)高频刷波动; 早盘(≤3天)仅抓开盘赔率一次(低频, 克制风控)。
PREMATCH_SCAN_SEC = 60        # 扫描周期
PREMATCH_WINDOW_SEC = 7200    # 临场档: kickoff 距今 ≤2h, 高频补采(波动最剧烈)
    ...
    def _prematch_backfill_tick(self) -> int:
        ...
        for mk, mid, ko_iso, st, minute in rows:
            ...
            if st == 'scheduled':
                dt = ko - now
                if is_focus:
                    picked.append((mk, str(mid), ko, -1, dt))            # 焦点场: 绕过窗口
                elif -300.0 <= dt <= PREMATCH_WINDOW_SEC:
                    picked.append((mk, str(mid), ko, 1, dt))             # 临场档: 高频刷波动
                elif 0.0 <= dt <= PREMATCH_EARLY_WINDOW_SEC:
                    picked.append((mk, str(mid), ko, 2, dt))             # 早盘档: 抓开盘赔率一次
            elif st == 'live':
        ...
        # 节流按档: 焦点场 30s(用户盯着); 临场/直播 45s; 早盘 6h(开盘后无需高频)。
        picked = [(mk, mid, ko, prio, now - (last.get(mk) or 0.0))
                  for mk, mid, ko, prio, _ in picked
                  if now - (last.get(mk) or 0.0) >= (
                      30.0 if prio == -1 else
                      PREMATCH_THROTTLE_SEC if prio in (0, 1) else
                      PREMATCH_EARLY_THROTTLE_SEC)]
        picked.sort(key=lambda p: (p[3], -p[4]))   # 焦点(-1) > 直播(0) > 临场(1) > 早盘(2); 同档断供最久先
